...
Public Class Form_registro_de_imagenes_de_productos_publicidad
    Dim MiPos As Integer
    Dim VarSeleccion As Integer
    Private Sub Form_registro_de_imagenes_de_productos_publicidad_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        Form_menu_principal.WindowState = FormWindowState.Maximized
    End Sub

    Private Sub Form_registro_de_imagenes_de_productos_publicidad_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyUp
    ...
    Private Sub Form_registro_de_imagenes_de_productos_publicidad_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        cargar_logo()
    ...
    Sub actualizar_tabla()
        ...
        SC.CommandText = "select * from productos_imagenes_sistema order by cod_auto asc"
        DA.SelectCommand = SC
        DA.Fill(DT)
        DS.Tables.Add(DT)
        conexion.Close()
    ...
    Sub mostrar(ByVal i As Integer)
        ...
        Try
            Dim Sql As String = "select imagen_producto from productos_imagenes_sistema where cod_auto='" & (txt_cod_auto.Text) & "'"
    ...
    Sub mostrar_datos_productos()
        ...
        Try
            Dim Sql As String = "select * from productos_imagenes_sistema where cod_auto='" & (txt_cod_auto.Text) & "'"
            Dim lector As MySqlDataReader
            conexion.Close()
            conexion.Open()
            If conexion.State = ConnectionState.Open Then
    ...
    Sub mostrar_datos_productos_para_modificar()
        ...
        If txt_codigo_producto.Text <> "" Then
            ...
            conexion.Open()
            SC2.Connection = conexion
            SC2.CommandText = "select * from productos_imagenes_sistema where cod_producto ='" & (txt_codigo_producto.Text) & "'"
            DA2.SelectCommand = SC2
            DA2.Fill(DT2)
            DS2.Tables.Add(DT2)
    ...
    Private Sub btn_guardar_1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_guardar.Click
        ...
        If VarSeleccion = 1 Then
            Try
                Dim sql As String = "INSERT INTO productos_imagenes_sistema (cod_producto,imagen_producto) VALUES ('" & (txt_codigo_producto.Text) & "',?imagen_producto);"
        ...
        If VarSeleccion = 2 Then
            Try
                Dim sql As String = "UPDATE productos_imagenes_sistema SET imagen_producto=?imagen_producto WHERE cod_auto='" & (txt_cod_auto.Text) & "'"
                Dim Comando As New MySqlCommand(sql, conexion)
                Dim Imag As Byte()
                Imag = Imagen_Bytes(Me.PictureBox1.Image)
                Comando.Parameters.AddWithValue("?imagen_producto", Imag)
    ...
    Private Sub btn_eliminar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_eliminar.Click
        If txt_cod_auto.Text <> "" Then
            ...
            If valormensaje = vbYes Then
                SC2.Connection = conexion
                SC2.CommandText = "DELETE FROM `productos_imagenes_sistema` WHERE `cod_auto`='" & (txt_cod_auto.Text) & "';"
                DA2.SelectCommand = SC2
                DA2.Fill(DT2)
                actualizar_tabla()
                mostrar(0)
