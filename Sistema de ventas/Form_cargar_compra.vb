...
Public Class Form_cargar_compra
    Dim dias As Integer

    Private Sub Form_cargar_compra_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        Form_Compra.Enabled = True
        ' Form_Compra.txt_nro_doc.Focus()
        Form_Compra.WindowState = FormWindowState.Normal

        If Form_Compra.combo_documento.Enabled = True Then
            Form_Compra.combo_documento.Focus()

        ElseIf Form_Compra.txt_codigo_producto.Enabled = True Then
            Form_Compra.txt_codigo_producto.Focus()
        Else
            Form_Compra.btn_nueva.Focus()
        End If
    End Sub

    Private Sub Form_cargar_compra_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyUp
    ...
    Private Sub Form_cargar_compra_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        cargar_logo()

        combo_documento.SelectedItem = "FACTURA ELECTRONICA"
    ...
    Sub cargar_datos_compra()
        ...
        If combo_documento.Text = "FACTURA ELECTRONICA" Then
            ...
            conexion.Open()
            SC.Connection = conexion
            SC.CommandText = "select * from compra where tipo='FACTURA' and tipo_emision='ELECTRONICA' and n_compra ='" & (txt_nro_doc.Text) & "' and rut_proveedor ='" & (txt_rut_proveedor.Text) & "'"
            DA.SelectCommand = SC
            DA.Fill(DT)
            DS.Tables.Add(DT)
            Try
                If DS.Tables(DT.TableName).Rows.Count > 0 Then
                    Form_Compra.combo_documento.Text = "FACTURA ELECTRONICA"
                    Form_Compra.txt_nro_doc.Text = DS.Tables(DT.TableName).Rows(0).Item("n_compra")
                    Form_Compra.txt_neto.Text = DS.Tables(DT.TableName).Rows(0).Item("neto")
                    Form_Compra.txt_iva.Text = DS.Tables(DT.TableName).Rows(0).Item("iva")
                    Form_Compra.txt_total.Text = DS.Tables(DT.TableName).Rows(0).Item("total")
                    Form_Compra.txt_codigo_compra.Text = DS.Tables(DT.TableName).Rows(0).Item("codigo_compra")
                    Form_Compra.dtp_emision.Text = DS.Tables(DT.TableName).Rows(0).Item("fecha_emision")
                    Form_Compra.txt_rut_proveedor.Text = txt_rut_proveedor.Text
                    Form_Compra.mostrar_datos_proveedores_combo()
                    Form_Compra.combo_documento.Enabled = False
                    Form_Compra.dtp_emision.Enabled = False
                    Form_Compra.txt_rut_proveedor.Enabled = False
                    Form_Compra.txt_nro_doc.Enabled = False
                    Form_Compra.txt_total.Enabled = False
                    Form_Compra.btn_modificar_doc.Enabled = True
                    Form_Compra.txt_nro_doc.Focus()
                Else
                    Form_Compra.txt_rut_proveedor.Text = txt_rut_proveedor.Text
        ...
        If combo_documento.Text = "FACTURA MANUAL" Then
            ...
            conexion.Open()
            SC.Connection = conexion
            SC.CommandText = "select * from compra where tipo='FACTURA' and tipo_emision='MANUAL' and n_compra ='" & (txt_nro_doc.Text) & "' and rut_proveedor ='" & (txt_rut_proveedor.Text) & "'"
            DA.SelectCommand = SC
            DA.Fill(DT)
            DS.Tables.Add(DT)
            Try
                If DS.Tables(DT.TableName).Rows.Count > 0 Then
                    Form_Compra.combo_documento.Text = "FACTURA MANUAL"
                    Form_Compra.txt_nro_doc.Text = DS.Tables(DT.TableName).Rows(0).Item("n_compra")
                    Form_Compra.txt_neto.Text = DS.Tables(DT.TableName).Rows(0).Item("neto")
                    Form_Compra.txt_iva.Text = DS.Tables(DT.TableName).Rows(0).Item("iva")
                    Form_Compra.txt_total.Text = DS.Tables(DT.TableName).Rows(0).Item("total")
        ...
        If combo_documento.Text = "GUIA ELECTRONICA" Then
            ...
            conexion.Open()
            SC.Connection = conexion
            SC.CommandText = "select * from compra where tipo='GUIA' and tipo_emision='ELECTRONICA' and n_compra ='" & (txt_nro_doc.Text) & "' and rut_proveedor ='" & (txt_rut_proveedor.Text) & "'"
            DA.SelectCommand = SC
            DA.Fill(DT)
            DS.Tables.Add(DT)
            Try
                If DS.Tables(DT.TableName).Rows.Count > 0 Then
                    Form_Compra.combo_documento.Text = "GUIA ELECTRONICA"
                    Form_Compra.txt_nro_doc.Text = DS.Tables(DT.TableName).Rows(0).Item("n_compra")
                    Form_Compra.txt_neto.Text = DS.Tables(DT.TableName).Rows(0).Item("neto")
                    Form_Compra.txt_iva.Text = DS.Tables(DT.TableName).Rows(0).Item("iva")
                    Form_Compra.txt_total.Text = DS.Tables(DT.TableName).Rows(0).Item("total")
        ...
        If combo_documento.Text = "GUIA MANUAL" Then
            ...
            conexion.Open()
            SC.Connection = conexion
            SC.CommandText = "select * from compra where tipo='GUIA' and tipo_emision='MANUAL' and n_compra ='" & (txt_nro_doc.Text) & "' and rut_proveedor ='" & (txt_rut_proveedor.Text) & "'"
            DA.SelectCommand = SC
            DA.Fill(DT)
            DS.Tables.Add(DT)
            Try
                If DS.Tables(DT.TableName).Rows.Count > 0 Then
                    Form_Compra.combo_documento.Text = "GUIA MANUAL"
                    Form_Compra.txt_nro_doc.Text = DS.Tables(DT.TableName).Rows(0).Item("n_compra")
                    Form_Compra.txt_neto.Text = DS.Tables(DT.TableName).Rows(0).Item("neto")
                    Form_Compra.txt_iva.Text = DS.Tables(DT.TableName).Rows(0).Item("iva")
                    Form_Compra.txt_total.Text = DS.Tables(DT.TableName).Rows(0).Item("total")
    ...
    Sub cargar_compra()
        Form_Compra.grilla_detalle.Rows.Clear()
        Form_Compra.Enabled = True
        conexion.Close()
        DS.Tables.Clear()
        DT.Rows.Clear()
        ...
        Form_Compra.grilla_detalle.Columns.Clear()
        Form_Compra.grilla_detalle.Columns.Add("cod_producto", "COD.")
        Form_Compra.grilla_detalle.Columns.Add("nombre", "NOMBRE")
        Form_Compra.grilla_detalle.Columns.Add("detalle_compras.cantidad", "CANT.")
        Form_Compra.grilla_detalle.Columns.Add("valor_unitario", "DESC. 1")
        Form_Compra.grilla_detalle.Columns.Add("descuento", "DESC. 2")
        Form_Compra.grilla_detalle.Columns.Add("descuento2", "DESC. 3")
        Form_Compra.grilla_detalle.Columns.Add("descuento3", "DESC. 4")
        Form_Compra.grilla_detalle.Columns.Add("neto", "NETO")

        Form_Compra.grilla_detalle.Columns(6).Visible = True
        Form_Compra.grilla_detalle.Columns(7).Visible = True
        Form_Compra.grilla_detalle.Columns(8).Visible = True
        Form_Compra.grilla_detalle.Columns(9).Visible = True
        Form_Compra.grilla_detalle.Columns(10).Visible = True
        Form_Compra.grilla_detalle.Columns(11).Visible = True
        Form_Compra.grilla_detalle.Columns(12).Visible = True
    ...
    Private Sub btn_aceptar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_aceptar.Click
        ...
        Form_Compra.controles(True, False)

        Form_Compra.combo_documento.Text = combo_documento.Text

        Form_Compra.cargar_compra()
        cargar_datos_compra()
        Form_Compra.mostrar_datos_proveedores()



        If Radio_impresora_1.Checked = True Then
            Form_Compra.Radio_impresora_1.Checked = True
        Else
            Form_Compra.Radio_impresora_2.Checked = True
        End If
        If radio_codigo_de_barra.Checked = True Then
            Form_Compra.radio_codigo_de_barra.Checked = True
        Else
            Form_Compra.radio_solo_numeros.Checked = True
        End If

        Form_Compra.txt_total.Focus()
        Me.Close()
    End Sub
