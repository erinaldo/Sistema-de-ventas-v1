...
Public Class Form_calcular_deuda_cliente
    Dim mifecha2 As String
    Dim mifecha4 As String
    Dim mifecha6 As String
    Private Sub Form_calcular_deuda_cliente_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed

        Form_menu_principal.WindowState = FormWindowState.Normal
    End Sub
    Private Sub Form_calcular_deuda_cliente_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyUp
    ...
    Sub mostrar_malla()
        ...
        If txt_rut.Text = "" Then
            ...
            'SC3.CommandText = "select TIPO, n_creditos, fecha_venta, total, saldo, clientes.rut_cliente, nombre_cliente, creditos.FECHA_VENTA, creditos.FECHA_vencimiento from creditos, clientes where TIPO <> 'ABONO' and TIPO <> 'NOTA DE CREDITO' and TIPO <> 'NOTA DE DEBITO' and saldo <> 0 and fecha_vencimiento >='" & (mifecha2) & "' and fecha_vencimiento <='" & (mifecha4) & "' and ESTADO <> 'ANULADA' and ESTADO <> 'ANULADO' and creditos.rut_cliente=clientes.rut_cliente group by n_creditos, clientes.rut_cliente order by fecha_vencimiento asc"
            SC3.CommandText = "select TIPO, n_creditos, fecha_venta, total, saldo, clientes.rut_cliente, nombre_cliente, creditos.FECHA_VENTA, creditos.FECHA_vencimiento from creditos, clientes where saldo <> 0 and fecha_vencimiento >='" & (mifecha2) & "' and fecha_vencimiento <='" & (mifecha4) & "' and ESTADO <> 'ANULADA' and ESTADO <> 'ANULADO' and creditos.rut_cliente=clientes.rut_cliente group by n_creditos, clientes.rut_cliente order by fecha_vencimiento asc"
            ...
            For i = 0 To grilla_deuda.Rows.Count - 1
                ...
                If fecha_pago <> "01-01-0001" And fecha_vencimiento <> "01-01-0001" Then
                    ...
                    dtp_pago.CustomFormat = "yyy-MM-dd"
                    dtp_vencimiento.CustomFormat = "yyy-MM-dd"
                    ' dias = DateDiff(DateInterval.Day, dtp_vencimiento.Value, dtp_pago.Value)

                    ' dias = DateDiff("d", CDate(dtp_pago.Value), CDate(dtp_vencimiento.Value))



                    Dim firstDate As String
                    Dim secondDate As Date
                    firstDate = dtp_vencimiento.Text
                    secondDate = CDate(firstDate)
                    dias = (DateDiff(DateInterval.Day, Now, secondDate))
                    dias = Math.Abs(dias)
                ...
                deuda = VarSaldo * (1 + dias * "0,00047")



                grilla_deuda_final.Rows.Add(VarDoc, VarNro, VarFecha, VarTotal, VarSaldo, VarRut, VarNombre, estado, fecha_vencimiento, fecha_pago, dias, deuda)
            Next
        ...
        Else
            ...
            conexion.Open()
            SC3.Connection = conexion
            'SC3.CommandText = "select TIPO, n_creditos, fecha_venta, total, saldo, clientes.rut_cliente, nombre_cliente, creditos.FECHA_VENTA, creditos.fecha_vencimiento  from creditos, clientes where TIPO <> 'ABONO' and TIPO <> 'NOTA DE CREDITO'  and TIPO <> 'NOTA DE CREDITO'and saldo <> 0  and TIPO <> 'NOTA DE DEBITO' and fecha_vencimiento >='" & (mifecha2) & "' and fecha_vencimiento <='" & (mifecha4) & "' and clientes.rut_cliente='" & (txt_rut.Text) & "' and ESTADO <> 'ANULADA' and ESTADO <> 'ANULADO' and creditos.rut_cliente=clientes.rut_cliente group by TIPO, n_creditos order by fecha_vencimiento asc"
            SC3.CommandText = "select TIPO, n_creditos, fecha_venta, total, saldo, clientes.rut_cliente, nombre_cliente, creditos.FECHA_VENTA, creditos.fecha_vencimiento  from creditos, clientes where saldo <> 0 and fecha_vencimiento >='" & (mifecha2) & "' and fecha_vencimiento <='" & (mifecha4) & "' and clientes.rut_cliente='" & (txt_rut.Text) & "' and ESTADO <> 'ANULADA' and ESTADO <> 'ANULADO' and creditos.rut_cliente=clientes.rut_cliente group by TIPO, n_creditos order by fecha_vencimiento asc"
            DA3.SelectCommand = SC3
            DA3.Fill(DT3)
            DS3.Tables.Add(DT3)
            grilla_deuda.DataSource = DS3.Tables(DT3.TableName)
            ...
            For i = 0 To grilla_deuda.Rows.Count - 1
                ...
                If fecha_pago <> "01-01-0001" And fecha_vencimiento <> "01-01-0001" Then
                    ...
                    dtp_vencimiento.CustomFormat = "yyy-MM-dd"
                    ' dias = DateDiff(DateInterval.Day, dtp_vencimiento.Value, dtp_pago.Value)
                    ' dias = DateDiff(DateInterval.Day, dtp_pago.Value, dtp_vencimiento.Value)
                    'dias = DateDiff("d", CDate(dtp_pago.Value), CDate(dtp_vencimiento.Value))
    ...
    Private Sub Form_calcular_deuda_cliente_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        cargar_logo()
    ...
    Sub grabar_deuda()
        ...
        SC2.CommandText = "DELETE FROM deuda_cliente WHERE cod_auto='1'"
        ...
        Dim VarRutCliente As String
        Dim VarDeudaCliente As String
        Dim VarNombreCliente As String
        For i = 0 To grilla_deuda_final.Rows.Count - 1

            VarRutCliente = grilla_deuda_final.Rows(i).Cells(5).Value.ToString
            VarNombreCliente = grilla_deuda_final.Rows(i).Cells(6).Value.ToString
            VarDeudaCliente = grilla_deuda_final.Rows(i).Cells(11).Value.ToString

            SC2.Connection = conexion
            SC2.CommandText = "insert into deuda_cliente (rut_cliente, nombre_cliente, deuda) values('" & (VarRutCliente) & "','" & (VarNombreCliente) & "','" & (VarDeudaCliente) & "')"
            DA2.SelectCommand = SC2
            DA2.Fill(DT2)
        ...
        grilla_deuda_clientes.DataSource = Nothing
        ...
        SC.Connection = conexion
        SC.CommandText = "select rut_cliente as 'RUT CLIENTE', nombre_cliente as 'NOMBRE CLIENTE', sum(deuda)  as ' DEUDA' from deuda_cliente GROUP BY rut_cliente ORDER BY DEUDA DESC"
        'SC.CommandText = "select deuda_cliente.rut_cliente as 'RUT CLIENTE', nombre_cliente as 'NOMBRE CLIENTE', sum(deuda)  as ' DEUDA', COUNT(registro_de_cobranzas.COD_AUTO) from deuda_cliente, registro_de_cobranzas WHERE registro_de_cobranzas.RUT_CLIENTE=deuda_cliente.RUT_CLIENTE GROUP BY deuda_cliente.rut_cliente ORDER BY DEUDA DESC"
        DA.SelectCommand = SC

        DA.Fill(DT)
        DS.Tables.Add(DT)

        grilla_deuda_clientes.DataSource = DS.Tables(DT.TableName)
        conexion.Close()
    End Sub
    ...
    Private Sub grilla_deuda11_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grilla_deuda_clientes.CellContentClick

    End Sub

    Private Sub grilla_deuda11_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles grilla_deuda_clientes.DoubleClick
        If grilla_deuda_clientes.Rows.Count = 0 Then
            Exit Sub
        End If
        Form_cobranzas.Show()
