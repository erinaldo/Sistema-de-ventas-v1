...
Public Class Form_buscar_proveedores_contactos

    Dim consulta_busqueda As String

    Private Sub Form_buscar_proveedores_contactos_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        cargar_logo()
        migrilla.Font = New System.Drawing.Font("Arial", 10.25!, System.Drawing.FontStyle.Regular)
        txt_busqueda.Focus()
    End Sub

    Private Sub Form_buscar_proveedores_contactos_KeyUp(sender As Object, e As KeyEventArgs) Handles Me.KeyUp
    ...
    Private Sub Form_buscar_proveedores_contactos_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Form_proveedores_contactos.WindowState = FormWindowState.Normal
        Form_proveedores_contactos.Enabled = True
    End Sub
    ...
    Private Sub migrilla_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles migrilla.DoubleClick
        ...
        If migrilla.Rows.Count <= 0 Then
            Form_proveedores_contactos.txt_rut.Focus()
            Me.Close()
        Else
            codigo = migrilla.CurrentRow.Cells(0).Value
            Form_proveedores_contactos.txt_rut.Text = codigo
    ...
    Sub MOSTRAR_DATOS_PROVEEDOR()
        ...
        If DS.Tables(DT.TableName).Rows.Count > 0 Then
            Form_proveedores_contactos.txt_rut.Text = DS.Tables(DT.TableName).Rows(0).Item("rut_proveedor")
            Form_proveedores_contactos.txt_nombre.Text = DS.Tables(DT.TableName).Rows(0).Item("nombre_proveedor")
            Form_proveedores_contactos.txt_mail.Text = DS.Tables(DT.TableName).Rows(0).Item("email_proveedor")
        End If
        conexion.Close()
