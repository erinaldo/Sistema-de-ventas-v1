...
Public Class Form_registro_proveedores_mas_pedidos
    Dim VarSeleccion As Integer
    Dim MiPos As Integer
    Private Sub Form_registro_proveedores_mas_pedidos_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles MyBase.FormClosed
        Form_menu_principal.WindowState = FormWindowState.Normal
    End Sub

    Private Sub Form_registro_proveedores_mas_pedidos_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyUp
    ...
    Private Sub Form_registro_proveedores_mas_pedidos_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        actualizar_tabla()
        mostrar(0)
    ...
    Sub mostrar(ByVal i As Integer)
        Try
            If DS.Tables(DT.TableName).Rows.Count > 0 Then
                txt_nombre_proveedor.Text = DS.Tables(DT.TableName).Rows(i).Item("nombre_proveedor")
            End If
        Catch
        End Try
    End Sub
    ...
    Sub controles(ByVal a As Boolean, ByVal b As Boolean)
        ...
        txt_nombre_proveedor.Enabled = a
    ...
    Private Sub btn_guardar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_guardar.Click
        ...
        If txt_nombre_proveedor.Text = "" Then mensaje = "CAMPO NOMBRE VACIO, FAVOR LLENAR" + Chr(13) & mensaje
        ...
        Else
            If VarSeleccion = 1 Then
                conexion.Close()
                Consultas_SQL("select * from proveedores_mas_pedidos where nombre_proveedor = '" & (txt_nombre_proveedor.Text) & "'")
                If DS.Tables(DT.TableName).Rows.Count > 0 Then
                ...
                Else
                    SC.CommandText = "INSERT INTO proveedores_mas_pedidos(nombre_proveedor) VALUES ('" & (txt_nombre_proveedor.Text) & "')"
                    DA.SelectCommand = SC
                    DA.Fill(DT)

                    SC.Connection = conexion
                    SC.CommandText = "insert into bitacora_de_cambios (procedencia, detalle, codigo, fecha, TIPO, usuario_responsable) values('PRODUCTOS','CREACION DE PROVEEDORES MAS PEDIDOS','" & (txt_nombre_proveedor.Text) & "','" & (Form_menu_principal.dtp_fecha.Text) & "','CREACION','" & (miusuario) & "')"
                    DA.SelectCommand = SC
                    DA.Fill(DT)
    ...
    Private Sub btn_eliminar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_eliminar.Click
        ...
        If valormensaje = vbYes Then
            SC.Connection = conexion
            SC.CommandText = "delete from proveedores_mas_pedidos where nombre_proveedor = '" & (txt_nombre_proveedor.Text) & "'"
            DA.SelectCommand = SC
            DA.Fill(DT)
        End If
    ...
    Sub actualizar_tabla()
        ...
        SC.Connection = conexion
        SC.CommandText = "select * from PROVEEDORES_MAS_PEDIDOS  where nombre_proveedor <> 'TODOS' order by nombre_proveedor"
        DA.SelectCommand = SC
        DA.Fill(DT)
        DS.Tables.Add(DT)
        conexion.Close()
    ...
    Private Sub txt_nombre_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txt_nombre_proveedor.GotFocus
        txt_nombre_proveedor.BackColor = Color.LightSkyBlue
    End Sub

    Private Sub txt_nombre_familia_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txt_nombre_proveedor.KeyPress
        'e.KeyChar = e.KeyChar.ToString.ToUpper
        e.KeyChar = e.KeyChar.ToString.ToUpper
    ...
    Private Sub txt_nombre_lostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txt_nombre_proveedor.LostFocus
