...
Public Class Form_consumo_interno
    ...
    Dim impresora_consumo_interno As String
    ...
    Sub mostrar_impresora()
        ...
        If DS.Tables(DT.TableName).Rows.Count > 0 Then
            impresora_consumo_interno = DS.Tables(DT.TableName).Rows(0).Item("ticket_consumo_interno")
        End If
        conexion.Close()
    End Sub
    ...
    Sub mostrar_malla()
        ...
        If DS.Tables(DT.TableName).Rows.Count > 0 Then
            ...
            For i = 0 To DS.Tables(DT.TableName).Rows.Count - 1
                ...
                grilla_consumo_interno.Rows.Add(DS.Tables(DT.TableName).Rows(i).Item("n_consumo_interno"),
                                                      ...
                                                      fecha_envio,
                                                       DS.Tables(DT.TableName).Rows(i).Item("hora"),
                                                        DS.Tables(DT.TableName).Rows(i).Item("nombre_usuario"),
                                                         DS.Tables(DT.TableName).Rows(i).Item("motivo"),
    ...
    Private Sub btn_agregar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_agregar.Click
        ...
        With Pd.PrinterSettings
            .PrinterName = impresora_consumo_interno
            .Copies = 1
            .PrintRange = PrintRange.AllPages
            If .IsValid Then
                Me.Pd.PrintController = New StandardPrintController
        ...
        If txt_medida.Text = "DECIMAL" Then
            ...
            cantidad_decimal = CDbl(cantidad_decimal) - CDbl(txt_cantidad.Text)

            SC.Connection = conexion
            SC.CommandText = "update productos set cantidad = '" & (cantidad_decimal) & "' where cod_producto = '" & (txt_codigo.Text) & "'"
            DA.SelectCommand = SC
            DA.Fill(DT)
        Else
            SC.Connection = conexion
    ...
    Private Sub btn_quitar_elemento_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_quitar_elemento.Click
        If grilla_consumo_interno.Rows.Count > 0 Then
            ...
            Try
                SC.Connection = conexion
                SC.CommandText = "DELETE FROM consumo_interno_temporal WHERE codigo_producto = '" & (codigo_consumo) & "' and n_consumo_interno = '" & (n_consumo_interno) & "' and cod_auto <> '0';"
                DA.SelectCommand = SC
                DA.Fill(DT)
            Catch
            End Try
            ...
            Try
                SC.Connection = conexion
                SC.CommandText = "DELETE FROM consumo_interno WHERE codigo_producto='" & (codigo_consumo) & "' and n_consumo_interno='" & (n_consumo_interno) & "' and cod_auto <> '0';"
                DA.SelectCommand = SC
                DA.Fill(DT)
            Catch
                MsgBox("ERROR AL ELIMINAR CONSUMO INTERNO", MessageBoxIcon.Information + MsgBoxStyle.OkOnly, "ATENCION")
            End Try

            Try
                SC.Connection = conexion
                SC.CommandText = "DELETE FROM detalle_consumo_interno WHERE TIPO='CONSUMO INTERNO' and cod_producto= '" & (codigo_consumo) & "' and n_documento= '" & (n_consumo_interno) & "' and cod_detalle <> '0';"
                DA.SelectCommand = SC
                DA.Fill(DT)
            Catch
                MsgBox("ERROR AL ELIMINAR CONSUMO INTERNO", MessageBoxIcon.Information + MsgBoxStyle.OkOnly, "ATENCION")
    ...
    Private Sub DocImprimir_PrintPage(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles Pd.PrintPage
        ...
        e.Graphics.DrawString("CANTIDAD", Font_texto_cabecera, Brushes.Black, margen_izquierdo + 270, margen_superior + 335, format1)
        e.Graphics.DrawLine(Pens.Black, margen_izquierdo + 1, margen_superior + 350, margen_izquierdo + 270, margen_superior + 350)
        ...
        e.Graphics.DrawString(txt_cantidad.Text, Font_texto_cabecera, Brushes.Black, margen_izquierdo + 270, margen_superior + 385, format1)

        e.Graphics.DrawLine(Pens.Black, margen_izquierdo + 1, margen_superior + 470, margen_izquierdo + 270, margen_superior + 470)
        Dim rect_firma As New Rectangle(margen_izquierdo + 10, margen_superior + 480, margen_izquierdo + 250, margen_superior + 15)
        e.Graphics.DrawString("FIRMA RESPONSABLE", Font_texto_titulo_detalle, Brushes.Black, rect_firma, stringFormat)


        Dim rect_guion_final As New Rectangle(margen_izquierdo + 10, margen_superior + 540, margen_izquierdo + 250, margen_superior + 15)
