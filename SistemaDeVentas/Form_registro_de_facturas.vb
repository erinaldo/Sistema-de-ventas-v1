...
Public Class Form_registro_de_facturas
    ...
    Private Sub Form_facturas_vencidas_2_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        fecha()
        fecha2()
        Combo_tipo_factura.SelectedItem = "ELECTRONICA"
    ...
    Sub mostrar_malla()
        ...
        If txt_nro_factura.Text <> "" Then
            consulta = "select documento, tipo_documento, nro_factura, total, fecha_factura, proveedores.nombre_proveedor as proveedor, libro_de_compras.rut_proveedor, estado, codigo_folio, periodo_tributario from libro_de_compras, proveedores where libro_de_compras.rut_proveedor=proveedores.rut_proveedor  "
        Else
            consulta = "select documento, tipo_documento, nro_factura, total, fecha_factura, proveedores.nombre_proveedor as proveedor, libro_de_compras.rut_proveedor, estado, codigo_folio, periodo_tributario from libro_de_compras, proveedores where libro_de_compras.rut_proveedor=proveedores.rut_proveedor and fecha_factura >='" & (mifecha2) & "' and fecha_factura <='" & (mifecha4) & "' "
        End If

        If txt_nro_factura.Text <> "" Then
            consulta = consulta & " and nro_factura='" & (txt_nro_factura.Text) & "'"
        End If

        If combo_estado.Text <> "TODOS" Then
            consulta = consulta & "  and estado='" & (combo_estado.Text) & "'"
        End If

        If txt_rut_proveedor.Text <> "" Then
            consulta = consulta & " and libro_de_compras.rut_proveedor='" & (txt_rut_proveedor.Text) & "'"
        End If

        If Combo_tipo_factura.Text <> "TODOS" Then
            consulta = consulta & " and tipo_documento='" & (Combo_tipo_factura.Text) & "'"
        End If
        ...
        If DS.Tables(DT.TableName).Rows.Count > 0 Then
            ...
            For i = 0 To DS.Tables(DT.TableName).Rows.Count - 1
                ...
                grilla_facturas.Rows.Add(documento, tipo, estado, proveedor, total, vencimiento, numero, rut_proveedor, codigo_folio, fecha_factura)
            Next
        ...
        For i = 0 To grilla_facturas.Rows.Count - 1
            estado_revision = grilla_facturas.Rows(i).Cells(2).Value.ToString
        ...
        If txt_rut_proveedor.Text = "" Then
            ...
            ElseIf combo_estado.Text <> "TODOS" Then
                ...
                SC.Connection = conexion
                SC.CommandText = "select documento, tipo_documento, nro_factura, total, fecha_factura, proveedores.nombre_proveedor as proveedor, LIBRO_DE_COMPRAS.RUT_PROVEEDOR, estado,codigo_folio, PERIODO_TRIBUTARIO from LIBRO_DE_compraS, proveedores where fecha_factura >='" & (mifecha2) & "' and fecha_factura <='" & (mifecha4) & "' and LIBRO_DE_compraS.rut_PROVEEDOR=proveedores.rut_PROVEEDOR  AND ESTADO='" & (combo_estado.Text) & "' and tipo_documento='" & (Combo_tipo_factura.Text) & "' group by NRO_FACTURA"
                DA.SelectCommand = SC
                DA.Fill(DT)
                DS.Tables.Add(DT)
    ...
    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_cambiar.Click
        ...
        Else
            ...
            For i = 0 To grilla_facturas.Rows.Count - 1
                If grilla_facturas.Rows(i).Cells(10).Value = True Then
    ...
    Sub calcular_totales()
        ...
        txt_total.Text = 0
        For i = 0 To grilla_facturas.Rows.Count - 1
            totalfacturas_adeudadas = Val(grilla_facturas.Rows(i).Cells(4).Value.ToString)
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_cargar.Click
        Form_cargar_detalle_registro_de_facturas.Show()
        Me.Enabled = False
    End Sub
    ...
    Private Sub grilla_facturas_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles grilla_facturas.Click
        If grilla_facturas.Rows.Count <= 0 Then
            Exit Sub
        End If
        txt_rut_proveedor.Text = grilla_facturas.CurrentRow.Cells(7).Value()
        '   End If
    End Sub
    ...
    Sub mostrar_datos_factura()
        ...
        documento = grilla_facturas.CurrentRow.Cells(0).Value()
        tipo_documento = grilla_facturas.CurrentRow.Cells(1).Value()
        rut_proveedor = grilla_facturas.CurrentRow.Cells(7).Value()
        nro_factura = grilla_facturas.CurrentRow.Cells(6).Value()
        estado = grilla_facturas.CurrentRow.Cells(2).Value()
        ...
        If estado = "PAGADA" Then
            ...
            SC.Connection = conexion
            SC.CommandText = "select fecha_de_pago,forma_de_pago, detalle_de_pago from libro_de_compras where documento= '" & (documento) & "' and  tipo_documento= '" & (tipo_documento) & "' and  rut_proveedor= '" & (rut_proveedor) & "' and nro_factura= '" & (nro_factura) & "'"
            DA.SelectCommand = SC
            DA.Fill(DT)
            DS.Tables.Add(DT)
    ...
    Private Sub PrintDocument1_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ...
        For i = 0 To grilla_facturas.Rows.Count - 1
            ...
            documento = grilla_facturas.Rows(i).Cells(0).Value.ToString
            tipo = grilla_facturas.Rows(i).Cells(1).Value.ToString
            estado = grilla_facturas.Rows(i).Cells(2).Value.ToString
            proveedor = grilla_facturas.Rows(i).Cells(3).Value.ToString
            total = grilla_facturas.Rows(i).Cells(4).Value.ToString
            vencimiento = grilla_facturas.Rows(i).Cells(5).Value.ToString
            numero = grilla_facturas.Rows(i).Cells(6).Value.ToString
            rut_proveedor = grilla_facturas.Rows(i).Cells(7).Value.ToString
            codigo_folio = grilla_facturas.Rows(i).Cells(8).Value.ToString
            fecha_factura = grilla_facturas.Rows(i).Cells(9).Value.ToString

            If documento = "FACTURA" Then
                e.Graphics.DrawString("F", Font2, Brushes.Black, 0, 35 + (i * 10))
            End If

            If documento = "NOTA DE CREDITO" Then
                e.Graphics.DrawString("NC", Font2, Brushes.Black, 0, 35 + (i * 10))
            End If

            Dim cadena As String
            cadena = fecha_factura
            fecha_factura = Mid(cadena, 1, 2)

            Dim TOTAL_MILLAR As String
            TOTAL_MILLAR = total
            TOTAL_MILLAR = Format(Int(total), "###,###,###")

            e.Graphics.DrawString(numero, Font2, Brushes.Black, 20, 35 + (i * 10))
    ...
    Private Sub Check_periodo_tributario_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Check_periodo_tributario.CheckedChanged
        If Check_periodo_tributario.Checked = True Then
            grilla_facturas.Columns(9).Visible = True
        Else
            grilla_facturas.Columns(9).Visible = False
        End If
    End Sub
    ...
    Private Sub Check_todos_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Check_todos.CheckedChanged
        If Check_todos.Checked = True Then
            For i = 0 To grilla_facturas.Rows.Count - 1
                grilla_facturas.Rows(i).Cells(10).Value = True
            Next
        Else
            For i = 0 To grilla_facturas.Rows.Count - 1
                grilla_facturas.Rows(i).Cells(10).Value = False
            Next
        End If
    End Sub
End Class