...
Public Class Form_nota_de_debito
    ...
    Private Sub Form_nota_de_debito_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        Form_menu_principal.WindowState = FormWindowState.Normal
    End Sub

    Private Sub Form_nota_de_debito_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyUp
    ...
    Private Sub Form_nota_de_debito_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Form_seleccionar_documento_para_nota_debito.Show()
        Me.Enabled = False
        Me.Width = 1024
        Me.Height = 728
        Centrar()
    ...
    Private Sub Form_venta_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
    ...
    '        btn_agregar.Location = New Point(801, 117)
    '        btn_quitar_elemento.Location = New Point(906, 117)
    '        txt_cantidad_agregar.Location = New Point(146, 123)
    '        '    txt_porcentaje_desc.Location = New Point(21, 25)
    '        lbl_cantidad_agregar.Location = New Point(-3, 126)
    ...
    '        Exit Sub
    '    ElseIf combo_venta.Text <> "BOLETA" And GroupBox_clientes.Enabled = False Then
    '        ' Me.Enabled = False
    '        GroupBox_producto.Location = New Point(0, 3)
    '        GroupBox_producto.Location = New Point(0, 112)
    '        btn_agregar.Location = New Point(801, 223)
    '        btn_quitar_elemento.Location = New Point(906, 223)
    '        txt_cantidad_agregar.Location = New Point(146, 232)
    '        '  txt_porcentaje_desc.Location = New Point(21, 25)
    '        lbl_cantidad_agregar.Location = New Point(-3, 235)
    ...
    Private Sub Form_venta_SizeChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.SizeChanged
        'ajustar()
    End Sub
    ...
    '    Panel_detalle.Size = New Point(Me.Width - 9, 32)
    ...
    Sub mostrar_datos_clientes()
        ...
        If txt_rut_cliente.Text <> "" Then
            ...
            ElseIf DS.Tables(DT.TableName).Rows.Count > 1 Then
                conexion.Close()
                Form_seleccionar_cuenta_nota_debito.Show()
                Me.Enabled = False
                Exit Sub
            ElseIf DS.Tables(DT.TableName).Rows.Count < 1 Then
    ...
    Sub limpiar()
        ...
        combo_condiciones.Text = "-"
        lbl_venta.Text = "NOTA DE DEBITO (EFECTIVO) $: 0"
        detalle_label()
    ...
    Sub crear_numero_documento()
        ...
        'Try
        '    SC.Connection = conexion
        '    SC.CommandText = "select max(n_nota_debito) as n_nota_debito from nota_debito where desglose <> 'MANUAL'"
        '    DA.SelectCommand = SC
        '    DA.Fill(DT)
        '    DS.Tables.Add(DT)
        '    If DS.Tables(DT.TableName).Rows.Count > 0 Then
        ...
        '        varnumdoc = DS.Tables(DT.TableName).Rows(0).Item("n_nota_debito")
        ...
        Try
            SC.Connection = conexion
            'SC.CommandText = "select max(cod_auto) as cod_auto from nota_debito where desglose <> 'MANUAL'"
        ...
        Try
            ...
            If DS.Tables(DT.TableName).Rows.Count > 0 Then
                ...
                txt_factura.Text = VarNumNotaDebito + 1
            End If
        Catch err As InvalidCastException
            txt_factura.Text = 1
        End Try
    ...
    Private Sub btn_agregar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_agregar.Click
        ...
        If grilla_detalle_venta.Rows.Count < 22 Then
            ...
            Exit Sub
        Else
            MsgBox("EN UNA NOTA DE DEBITO NO PUEDE AGREGAR MAS DE 22 ITEMS", 0 + 16, "ERROR")
            Exit Sub
        End If
    ...
    Sub calcular_totales()
        ...
        '  If finTotal > 0 And finTotal < 10 Then
        'valor_total = valor_total - numerofinal
        ' End If

        'If finTotal > 10 Then
        '    finTotal = finTotal - 10
    ...
    Sub imprimir()
        ...
        SC4.CommandText = "select * from nota_debito where n_nota_debito ='" & (txt_factura.Text) & "'"
        ...
        valormensaje = MsgBox("¿ESTA SEGURO DE IMPRIMIR UNA NOTA DE DEBITO?", MsgBoxStyle.YesNo + MsgBoxStyle.Question, "INFORMACION")
    ...
    Sub grabar_detalle_factura()
        ...
        For i = 0 To grilla_detalle_venta.Rows.Count - 1
            ...
            SC.Connection = conexion
            SC.CommandText = "insert into detalle_nota_debito (n_nota_debito, cod_producto, detalle_nombre, numero_tecnico, valor_unitario, cantidad, porcentaje_desc, detalle_descuento, detalle_neto, detalle_iva, detalle_subtotal, detalle_total ) values(" & (txt_factura.Text) & ",'" & (VarCodProducto) & "','" & (varnombre) & "','" & (vartecnico) & "'," & (VarValorUnitario) & ",'" & (VarCantidad) & "'," & (VarPorcentaje) & "," & (VarDescuento) & "," & (VarNeto) & ", " & (VarIva) & "," & (VarSubtotal) & "," & (VarTotal) & ")"
            DA.SelectCommand = SC
            DA.Fill(DT)
            conexion.Close()
    ...
    Sub grabar_factura()
        ...
        If estado_nota_de_debito_electronica = "SI" Then
    ...
    Private Sub btn_grabar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_grabar.Click
        ...
        If Combo_codigo_referencia.Text = "ANULA DOCUMENTO DE REFERENCIA" And txt_total_doc_referencia.Text <> valor_total Then
            MsgBox("EL TOTAL DE LA NOTA DE DEBITO ES DISTINTO AL TOTAL DEL DOC. DE REFERENCIA", MessageBoxIcon.Information + MsgBoxStyle.OkOnly, "ATENCION")
            txt_codigo.Focus()
            Exit Sub
        End If

        If txt_tipo_doc_referencia.Text = "FACTURA" And txt_rut_doc_referencia.Text <> txt_rut_cliente.Text Then
            MsgBox("EL RUT DE LA NOTA DE DEBITO ES DISTINTO AL RUT DE LA FACTURA", MessageBoxIcon.Information + MsgBoxStyle.OkOnly, "ATENCION")
            txt_codigo.Focus()
            Exit Sub
        End If
    ...
    Sub detalle_label()
        ...
        If txt_total.Text <> "" And txt_total.Text <> "0" Then
            ...
            lbl_venta.Text = "NOTA DE DEBITO $: " & (total_millar)
        Else
            lbl_venta.Text = "NOTA DE DEBITO $: 0"
        End If
    End Sub
    ...
    Sub crear_archivo_plano()
        ...
        Try
            ...
            ' PathArchivo = "\\SERVER\Claudio Calderon R\Capeta\Archivo '" & (combo_venta.Text) & "'" & Format(Today.Date, "ddMMyyyy") & ".txt" ' Se determina el nombre del archivo con la fecha actual
            ' PathArchivo = "C:\Carpeta Facturacion Electronica\" & (combo_venta.Text) & " " & (txt_factura.Text) & ".txt" ' Se determina el nombre del archivo con la fecha actual
            PathArchivo = ruta_archivos_planos & "\" & "Nota de debito " & (txt_factura.Text) & ".txt" ' Se determina el nombre del archivo con la fecha actual
            ...
            strStreamWriter.WriteLine("->Encabezado<-" & vbNewLine _
                                       & "56" & ";" & (txt_factura.Text) & ";" & (dtp_ingreso.Text) & ";" & "0" & ";" & "0" & ";" & (txt_rut_cliente.Text) & ";" & (nombre_cliente) & ";" & (giro_cliente) & ";" & (direccion_cliente) & ";" & (ciudad_cliente) & ";" & (comuna_cliente) & ";" & (correo_cliente) & ";" & (condicion) & ";" & (Combo_vendedor.Text) & ";;;" & (mirecintoempresa) & ";" & (telefono_cliente) & ";" & vbNewLine _
                                       & "->Totales<-" & vbNewLine _
