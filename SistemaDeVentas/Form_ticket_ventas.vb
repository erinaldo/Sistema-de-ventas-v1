...
Imports System.IO
Imports System.Math
Public Class Form_ticket_ventas
    ...
    Private Sub Form_ticket_ventas_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        Form_menu_principal.WindowState = FormWindowState.Normal
    End Sub

    Private Sub Form_ticket_ventas_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyUp
    ...
    Private Sub Form_ticket_ventas_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        cargar_logo()
    ...
    Sub mostrar_malla_detalle()
        ...
        DS.Clear()
        SC.Connection = conexion
        SC.CommandText = "select * from detalle_ticket_venta where n_ticket= '" & (nro_doc) & "'"
    ...
    Sub mostrar_malla_credito()
        ...
        If condicion_doc = "LETRA" Then
            ...
            DS.Clear()
            SC.Connection = conexion
            SC.CommandText = "select * from cuotas_temporal where nro_ticket= '" & (nro_doc) & "'"
            ...
            If DS.Tables(DT.TableName).Rows.Count > 0 Then
                ...
                For i = 0 To DS.Tables(DT.TableName).Rows.Count - 1
                    grilla_cuotas.Rows.Add(DS.Tables(DT.TableName).Rows(i).Item("nro_cuota"), _
                                            DS.Tables(DT.TableName).Rows(i).Item("fecha"), _
                                             DS.Tables(DT.TableName).Rows(i).Item("total"), _
                                              DS.Tables(DT.TableName).Rows(i).Item("amortizacion"), _
        ...
        grilla_cuotas.Columns(3).Visible = False
        grilla_cuotas.Columns(4).Visible = False
        grilla_cuotas.Columns(5).Visible = False
    End Sub


    Sub mostrar_malla_detalle_de_pago()
        ...
        DS.Clear()
        SC.Connection = conexion
        SC.CommandText = "select * from detalle_condicion_de_pago_temporal where nro_doc= '" & (nro_doc) & "' and tipo_doc= '" & (tipo_doc) & "'"
    ...
    Private Sub grilla_documento_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles grilla_documento.Click
        ...
        mostrar_malla_credito()

        mostrar_malla_detalle_de_pago()
    ...
    Sub crear_numero_letra()
        ...
        If condicion_doc = "LETRA" Then
            ...
            Try
                SC.Connection = conexion
                SC.CommandText = "select n_letra from letras where cod_auto='" & (varnumdoc) & "'"
                DA.SelectCommand = SC
                DA.Fill(DT)
                DS.Tables.Add(DT)
                If DS.Tables(DT.TableName).Rows.Count > 0 Then
                    varnumdoc = DS.Tables(DT.TableName).Rows(0).Item("n_letra")
                    txt_nro_letra.Text = varnumdoc + 1
                End If
            Catch err As InvalidCastException
                txt_nro_letra.Text = 1
            End Try
        ...
        If condicion_doc = "CONVENIO" Then
            ...
            Try
                SC.Connection = conexion
                SC.CommandText = "select max(cod_auto) as cod_auto from cuotas"
                DA.SelectCommand = SC
                DA.Fill(DT)
                DS.Tables.Add(DT)
                If DS.Tables(DT.TableName).Rows.Count > 0 Then
            ...
            Try
                SC.Connection = conexion
                SC.CommandText = "select n_cuota from cuotas where cod_auto='" & (varnumdoc) & "'"
                DA.SelectCommand = SC
                DA.Fill(DT)
                DS.Tables.Add(DT)
                If DS.Tables(DT.TableName).Rows.Count > 0 Then
                    varnumdoc = DS.Tables(DT.TableName).Rows(0).Item("n_cuota")
                    txt_nro_letra.Text = varnumdoc + 1
                End If
            Catch err As InvalidCastException
                txt_nro_letra.Text = 1
    ...
    Sub grabar_factura()
        ...
        If tipo_doc = "GUIA" Then
            ...
            SC.Connection = conexion
            SC.CommandText = "insert into guia (caja, n_guia, tipo, rut_cliente, codigo_cliente, fecha_venta, descuento, neto, iva, subtotal, total, condiciones, estado, usuario_responsable,rut_retira, nombre_retira, recinto, orden, hora, porcentaje_desc, tipo_impresion, pie, condicion_de_pago_pie) values ('" & (nombre_pc) & "', " & (txt_factura.Text) & " , '" & ("GUIA") & "', '" & (rut_cliente) & "','" & (cod_cliente) & "','" & (fecha_doc) & "'," & (descuento_doc) & "," & (neto_doc) & "," & (iva_doc) & "," & (subtotal_doc) & "," & (total_doc) & ",'" & (condicion_doc) & "','" & ("SIN FACTURAR") & "','" & (rut_vendedor) & "','" & (rut_retira) & "','" & (nombre_retira) & "','" & (mirecintoempresa) & "','" & ("0") & "','" & (Form_menu_principal.lbl_hora.Text) & "','" & (porcentaje_desc_doc) & "', '" & (tipo_impresion) & "', '" & (pie) & "', '" & (forma_de_pago_pie) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)
        End If
        ...
        If tipo_doc = "BOLETA" Then
            ...
            End If
            SC.Connection = conexion
            SC.CommandText = "insert into BOLETA (caja, n_boleta, tipo, rut_cliente, codigo_cliente, fecha_venta, descuento, neto, iva,  subtotal, total, condiciones,estado, usuario_responsable, hora, porcentaje_desc, tipo_impresion, pie, condicion_de_pago_pie) values ('" & (nombre_pc) & "', " & (txt_factura.Text) & " , 'BOLETA','" & (rut_cliente) & "','" & (cod_cliente) & "','" & (fecha_doc) & "'," & (descuento_doc) & "," & (neto_doc) & "," & (iva_doc) & "," & (subtotal_doc) & "," & (total_doc) & ",'" & (condicion_doc) & "','" & ("EMITIDA") & "','" & (miusuario) & "','" & (Form_menu_principal.lbl_hora.Text) & "','" & (porcentaje_desc_doc) & "', '" & (tipo_impresion) & "', '" & (pie) & "', '" & (forma_de_pago_pie) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)
        End If
        ...
        If tipo_doc = "BOLETA DE CREDITO" Then
            ...
            SC.Connection = conexion
            SC.CommandText = "insert into BOLETA (caja, n_boleta, tipo, rut_cliente,  codigo_cliente,  fecha_venta, descuento, neto, iva, subtotal, total, condiciones, estado, usuario_responsable, hora, porcentaje_desc, tipo_impresion, pie, condicion_de_pago_pie) values ('" & (nombre_pc) & "', " & (txt_factura.Text) & " , '" & ("BOLETA DE CREDITO") & "', '" & (rut_cliente) & "','" & (cod_cliente) & "','" & (fecha_doc) & "'," & (descuento_doc) & "," & (neto_doc) & "," & (iva_doc) & "," & (subtotal_doc) & "," & (total_doc) & ",'" & (condicion_doc) & "','" & ("EMITIDA") & "','" & (rut_vendedor) & "','" & (Form_menu_principal.lbl_hora.Text) & "','" & (porcentaje_desc_doc) & "', '" & (tipo_impresion) & "', '" & (pie) & "', '" & (forma_de_pago_pie) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)
        ...
        If tipo_doc = "FACTURA" Then
            ...
            SC.Connection = conexion
            SC.CommandText = "insert into factura(caja, n_factura,TIPO,rut_cliente,  codigo_cliente, fecha_venta,descuento,neto,iva,subtotal,total,condiciones,estado,usuario_responsable,rut_retira, nombre_retira, recinto, hora, porcentaje_desc, orden, tipo_impresion, pie, condicion_de_pago_pie)values('" & (nombre_pc) & "', " & (txt_factura.Text) & ",'" & ("FACTURA") & "','" & (rut_cliente) & "','" & (cod_cliente) & "','" & (fecha_doc) & "'," & (descuento_doc) & "," & (neto_doc) & "," & (iva_doc) & "," & (subtotal_doc) & "," & (total_doc) & ",'" & (condicion_doc) & "','" & ("EMITIDA") & "','" & (miusuario) & "','" & (rut_retira) & "','" & (nombre_retira) & "','" & (mirecintoempresa) & "','" & (Form_menu_principal.lbl_hora.Text) & "','" & (porcentaje_desc_doc) & "', '0', '" & (tipo_impresion) & "', '" & (pie) & "', '" & (forma_de_pago_pie) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)
        End If
    ...
    Sub grabar_detalle_factura()
        ...
        If combo_condiciones.Text = "LETRA" Then
            ...
            'Dim total_credito As Integer
            'total_credito = Val(total_doc) - Val(pie)

            'SC.Connection = conexion
            'SC.CommandText = "insert into detalle_condicion_de_pago (nro_doc, tipo_doc, valor, condicion_de_pago, estado, fecha) values(" & (txt_factura.Text) & ", '" & (VarTipoDoc) & "', '" & (total_credito) & "', 'CREDITO', 'EMITIDA', '" & (form_Menu_admin.dtp_fecha.Text) & "')"
            'DA.SelectCommand = SC
            'DA.Fill(DT)

            'If Val(pie) <> 0 Then
            '    SC.Connection = conexion
            '    SC.CommandText = "insert into detalle_condicion_de_pago (nro_doc, tipo_doc, valor, condicion_de_pago, estado, fecha) values(" & (txt_factura.Text) & ", '" & (VarTipoDoc) & "', '" & (total_credito) & "', '" & (forma_de_pago_pie) & "', 'EMITIDA', '" & (form_Menu_admin.dtp_fecha.Text) & "')"
            '    DA.SelectCommand = SC
            '    DA.Fill(DT)
            'End If
        ...
        If combo_condiciones.Text = "CONVENIO" Then
            ...
            'If Val(pie) <> 0 Then
            '    SC.Connection = conexion
            '    SC.CommandText = "insert into detalle_condicion_de_pago (nro_doc, tipo_doc, valor, condicion_de_pago, estado, fecha) values(" & (txt_factura.Text) & ", '" & (VarTipoDoc) & "', '" & (pie) & "', '" & (forma_de_pago_pie) & "', 'EMITIDA', '" & (form_Menu_admin.dtp_fecha.Text) & "')"
            '    DA.SelectCommand = SC
            '    DA.Fill(DT)
            'End If
    ...
    Sub crear_archivo_plano()
        ...
        If condicion_doc = "EFECTIVO" Then
            condicion = "CONTADO " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "ABCDIN" Then
            condicion = "CONTADO " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "CENCOSUD" Then
            condicion = "CONTADO " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "C&D" Then
            condicion = "CONTADO " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "PRESTO" Then
            condicion = "CONTADO " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "RYPLEY" Then
            condicion = "CONTADO " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "VISA" Then
            condicion = "CONTADO " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "MASTERCARD" Then
            condicion = "CONTADO " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "BANCARIA" Then
            condicion = "CONTADO " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "REDCOMPRA" Then
            condicion = "CONTADO " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "TRANSFERENCIA" Then
            condicion = "CONTADO " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "CHEQUE AL DIA" Then
            condicion = "DOCUMENTADA " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "CHEQUE 15 DIAS" Then
            condicion = "DOCUMENTADA " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "CHEQUE 30 DIAS" Then
            condicion = "DOCUMENTADA " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "CHEQUE 45 DIAS" Then
            condicion = "DOCUMENTADA " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "CHEQUE 60 DIAS" Then
            condicion = "DOCUMENTADA " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "CHEQUE 30-60 DIAS" Then
            condicion = "DOCUMENTADA " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "CHEQUE 30-60-90 DIAS" Then
            condicion = "DOCUMENTADA " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "PENDIENTE" Then
            condicion = "DOCUMENTADA " & "(" & condicion_doc & ")"
        End If
        If condicion_doc = "TRANSFERENCIA" Then
            condicion = "CONTADO " & "(" & condicion_doc & ")"
        ...
        If condicion_doc = "CREDITO" Then
            condicion = "CREDITO " & "(" & condicion_doc & ")"
        End If
        ...
        If tipo_doc = "FACTURA" Then
            Try
                ...
                strStreamWriter.WriteLine("->Encabezado<-" & vbNewLine _
                                           & "33" & ";" & (txt_factura.Text) & ";" & (fecha_doc) & ";" & "0" & ";" & "0" & ";" & (rut_cliente) & ";" & (nombre_cliente) & ";" & (giro_cliente) & ";" & (direccion_cliente) & ";" & (ciudad_cliente) & ";" & (comuna_cliente) & ";" & (correo_cliente) & ";" & (condicion) & ";" & (minombre) & ";" & (rut_retira) & ";" & (nombre_retira) & ";" & (mirecintoempresa) & ";" & (telefono_cliente) & ";" & vbNewLine _
                                           & "->Totales<-" & vbNewLine _
                ...
                For i = 0 To grilla_detalle_documento.Rows.Count - 1
                    ...
                    strStreamWriter.WriteLine(nro_linea & ";" & (VarCodProducto) & ";" & (varnombre) & ";" & (VarCantidad) & ";" & (VarPrecioLista) & ";" & (VarPorcentaje) & ";" & (VarDescuento) & ";" & (0) & ";" & (0) & ";" & (0) & ";" & (VarTotal) & ";" & "INT11" & ";" & "UN" & ";" & ";")
                Next
    ...
    Private Sub DocImprimir_PrintPage(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles Pd.PrintPage
        ...
        If impresion_de_documento = "LETRA" Then
            ...
            e.Graphics.DrawString(Format(Form_menu_principal.dtp_fecha.Text, "Long Date"), Font10, Brushes.Black, 310, 30)
    ...
    Sub imprimir_creditos()
        ...
        If condicion_doc = "CONVENIO" Then
            ...
            lbl_mensaje.Visible = True
            Me.Enabled = False
            For i = 0 To grilla_cuotas.Rows.Count - 1
                VarNroCuota = grilla_cuotas.Rows(i).Cells(0).Value.ToString
                varFechaCuota = grilla_cuotas.Rows(i).Cells(1).Value.ToString
                varTotalCuota = grilla_cuotas.Rows(i).Cells(2).Value.ToString
                VarAmotizacionCuota = grilla_cuotas.Rows(i).Cells(3).Value.ToString
                VarInteresCuota = grilla_cuotas.Rows(i).Cells(4).Value.ToString
                VarSaldoCuota = grilla_cuotas.Rows(i).Cells(5).Value.ToString
                dtp_hasta.Text = dtp_hasta.Value.AddMonths(Val(+1))
                crear_numero_letra()
                grabar_letras()
        ...
        impresion_de_documento = "LETRA"
        If impresora_reportes <> "SIN IMPRESION" Then
            If condicion_doc = "LETRA" Then
                dtp_hasta.Text = FormatDateTime(Now, DateFormat.ShortDate)
                cantidad_letras = 1
                With Pd.PrinterSettings
                    ' Especifico el nombre de la impresora 
                    ' por donde deseo imprimir. 
                    .PrinterName = impresora_reportes
                    ' Establezco el número de copias que se imprimirán 
                    .Copies = 1
                    If .IsValid Then
                        lbl_mensaje.Visible = True
                        ...
                        For i = 0 To grilla_cuotas.Rows.Count - 1
                            ...
                            With Pd.PrinterSettings
                                ...
                                If .IsValid Then
                                    ...
                                    Me.Pd.DefaultPageSettings.Margins.Left = 10
                                    Me.Pd.DefaultPageSettings.Margins.Right = 10
                                    Dim pkCustomSize1 As New PaperSize("New Long Roll", 1000, 550)
                                    Me.Pd.DefaultPageSettings.PaperSize = pkCustomSize1
                                    Pd.PrintController = New System.Drawing.Printing.StandardPrintController()
                                    Pd.Print()
                                    cantidad_letras = cantidad_letras + 1
    ...
    '    dr("fecha_palabras") = Format(Form_menu_principal.dtp_fecha.Text, "Long Date")
    ...
    Sub grabar_letras()
        ...
        Dim fecha_cuota As String
        Dim total_cuota As String
        Dim amortizacion_cuota As String
        Dim interes_cuota As String
        Dim saldo_cuota As String
        '  Dim pie As String


        nro_cuota = Me.grilla_cuotas.CurrentRow.Cells(0).Value
        fecha_cuota = Me.grilla_cuotas.CurrentRow.Cells(1).Value
        total_cuota = Me.grilla_cuotas.CurrentRow.Cells(2).Value
        amortizacion_cuota = Me.grilla_cuotas.CurrentRow.Cells(3).Value
        interes_cuota = Me.grilla_cuotas.CurrentRow.Cells(4).Value
        saldo_cuota = Me.grilla_cuotas.CurrentRow.Cells(5).Value
        '  pie = Me.grilla_cuotas.CurrentRow.Cells(6).Value
        ...
        Neto_letra = Round(varTotalCuota / iva_valor)
        Iva_letra = (((Neto_letra) * valor_iva) / 100)
        Iva_letra = (varTotalCuota) - (Neto_letra)



        If condicion_doc = "LETRA" Then
            SC.Connection = conexion
            SC.CommandText = "insert into  creditos (n_creditos, TIPO, tipo_detalle, rut_cliente, codigo_cliente, fecha_venta, descuento, neto, iva, subtotal, total, saldo, condiciones, estado, usuario_responsable, codigo_afecta, nombre_afecta, recinto, fecha_vencimiento, fecha_pago, numero_cuota, total_cuotas, interes, gastos_de_cobranza, pie, convenio) values (" & (txt_nro_letra.Text) & ",'" & ("LETRA") & "','" & (Me.combo_venta.Text & " " & txt_factura.Text & " " & VarNroCuota & " DE " & grilla_cuotas.Rows.Count) & "','" & (rut_cliente) & "','" & (cod_cliente) & "','" & (Form_menu_principal.dtp_fecha.Text) & "','0'," & (Neto_letra) & "," & (Iva_letra) & "," & (varTotalCuota) & "," & (varTotalCuota) & "," & (varTotalCuota) & ",'CREDITO','" & ("EMITIDA") & "','" & (miusuario) & "','" & (txt_nro_letra.Text) & "','Letra', '" & (mirecintoempresa) & "', '" & (varFechaCuota) & "', '0000-00-00', '" & (VarNroCuota) & "', '" & (grilla_cuotas.Rows.Count) & "', '0', '0', '" & (pie) & "','" & (convenio) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)

            SC.Connection = conexion
            SC.CommandText = "insert into letras (n_letra, cant_letras, doc_referencia, nro_referencia, fecha, rut_cliente, monto_letra, total_letra, estado, usuario_responsable, fecha_vencimiento, orden_letras, total_doc_referencia) values (" & (txt_nro_letra.Text) & " , '" & (grilla_cuotas.Rows.Count) & "', '" & (combo_venta.Text) & "','" & (txt_factura.Text) & "','" & (Form_menu_principal.dtp_fecha.Text) & "','" & (rut_cliente) & "'," & (varTotalCuota) & "," & (varTotalCuota) & ", 'EMITIDA', '" & (miusuario) & "','" & (varFechaCuota) & "','" & (VarNroCuota) & "','" & (varTotalCuota) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)
        End If
        ...
        If condicion_doc = "CONVENIO" Then
            SC.Connection = conexion
            SC.CommandText = "insert into creditos (n_creditos, TIPO, tipo_detalle, rut_cliente, codigo_cliente, fecha_venta, descuento, neto, iva, subtotal, total, saldo, condiciones, estado, usuario_responsable, codigo_afecta, nombre_afecta, recinto, fecha_vencimiento, fecha_pago, numero_cuota, total_cuotas, interes, gastos_de_cobranza, pie, convenio) values (" & (txt_nro_letra.Text) & ",'" & ("CUOTA") & "','" & (Me.combo_venta.Text & " " & txt_factura.Text & " " & VarNroCuota & " DE " & grilla_cuotas.Rows.Count) & "','" & (rut_cliente) & "','" & (cod_cliente) & "','" & (Form_menu_principal.dtp_fecha.Text) & "','0'," & (Neto_letra) & "," & (Iva_letra) & "," & (varTotalCuota) & "," & (varTotalCuota) & "," & (varTotalCuota) & ",'CREDITO','" & ("EMITIDA") & "','" & (miusuario) & "','" & (txt_nro_letra.Text) & "','CUOTA', '" & (mirecintoempresa) & "', '" & (varFechaCuota) & "', '0000-00-00', '" & (VarNroCuota) & "', '" & (grilla_cuotas.Rows.Count) & "', '0', '0', '" & (pie) & "','" & (convenio) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)

            SC.Connection = conexion
            SC.CommandText = "insert into CUOTAS (n_cuota, cant_cuota, doc_referencia, nro_referencia, fecha, rut_cliente, monto_cuota, total_cuota, estado, usuario_responsable, fecha_vencimiento, orden_cuota, total_doc_referencia) values (" & (txt_nro_letra.Text) & " , '" & (grilla_cuotas.Rows.Count) & "', '" & (combo_venta.Text) & "','" & (txt_factura.Text) & "','" & (Form_menu_principal.dtp_fecha.Text) & "','" & (rut_cliente) & "'," & (varTotalCuota) & "," & (varTotalCuota) & ", 'EMITIDA', '" & (miusuario) & "','" & (varFechaCuota) & "','" & (VarNroCuota) & "','" & (varTotalCuota) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)
        End If
    End Sub
    ...
    Private Sub grilla_cuotas_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grilla_cuotas.CellContentClick

    End Sub

    Private Sub txt_vuelto_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txt_vuelto.TextChanged
