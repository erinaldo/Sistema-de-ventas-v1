...
Public Class Form_envio_productos_sucursal
    ...
    Sub mostrar_malla()
        ...
        If DS.Tables(DT.TableName).Rows.Count > 0 Then
            ...
            For i = 0 To DS.Tables(DT.TableName).Rows.Count - 1
                grilla_entra.Rows.Add(DS.Tables(DT.TableName).Rows(i).Item("codigo_producto"),
                                                 DS.Tables(DT.TableName).Rows(i).Item("nombre"),
                                                  DS.Tables(DT.TableName).Rows(i).Item("nro_tecnico"),
                                                    ...
                                                    DS.Tables(DT.TableName).Rows(i).Item("cantidad"),
                                                     fecha_recibo,
                                                      DS.Tables(DT.TableName).Rows(i).Item("hora"),
                                                       DS.Tables(DT.TableName).Rows(i).Item("despachador"),
                                                        DS.Tables(DT.TableName).Rows(i).Item("nombre_usuario"),
                                                         DS.Tables(DT.TableName).Rows(i).Item("n_vale"),
                                                          DS.Tables(DT.TableName).Rows(i).Item("sucursal"),
                                                           DS.Tables(DT.TableName).Rows(i).Item("nombre"),
                                                            DS.Tables(DT.TableName).Rows(i).Item("aplicacion"),
                                                             DS.Tables(DT.TableName).Rows(i).Item("nro_tecnico"),
                                                              DS.Tables(DT.TableName).Rows(i).Item("marca"),
                                                               DS.Tables(DT.TableName).Rows(i).Item("cod_barra"),
                                                                DS.Tables(DT.TableName).Rows(i).Item("familia"),
                                                                 DS.Tables(DT.TableName).Rows(i).Item("subfamilia"),
                                                                  DS.Tables(DT.TableName).Rows(i).Item("subfamilia2"),
                                                                   DS.Tables(DT.TableName).Rows(i).Item("cantidad_minima"),
                                                                     ...
                                                                     DS.Tables(DT.TableName).Rows(i).Item("tipo_precio"),
                                                                      DS.Tables(DT.TableName).Rows(i).Item("factor"),
                                                                       DS.Tables(DT.TableName).Rows(i).Item("costo"),
                                                                        DS.Tables(DT.TableName).Rows(i).Item("rut_proveedor"),
                                                                         DS.Tables(DT.TableName).Rows(i).Item("fecha_ult_compra"),
                                                                          DS.Tables(DT.TableName).Rows(i).Item("cant_ult_compra"),
                                                                           DS.Tables(DT.TableName).Rows(i).Item("tipo_doc"),
    ...
    Private Sub btn_agregar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_agregar.Click
        ...
        With Pd.PrinterSettings
            ...
            If .IsValid Then
                Me.Pd.PrintController = New StandardPrintController
                Dim pkCustomSize1 As New PaperSize("New Long Roll", 1000, 700)
                Me.Pd.DefaultPageSettings.PaperSize = pkCustomSize1
                Pd.PrintController = New System.Drawing.Printing.StandardPrintController()
                Pd.Print()
            Else
    ...
    Private Sub btn_agregar_entra_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_agregar_entra.Click
        ...
        If grilla_entra.Rows.Count > 0 Then
            For i = 0 To grilla_entra.Rows.Count - 1
                If grilla_entra.Rows(i).Cells(31).Value = True And grilla_entra.Rows(i).Cells(29).Value <> "RECIBIDO" Then
                    Form_autorizacion_traspaso.Show()
                    Me.Enabled = False
                    Exit Sub
                End If
        ...
        If Check_todos.Checked = True Then
            For i = 0 To grilla_entra.Rows.Count - 1
                If grilla_entra.Rows(i).Cells(29).Value <> "RECIBIDO" Then
                    grilla_entra.Rows(i).Cells(31).Value = True
                End If
            Next
        Else
            For i = 0 To grilla_entra.Rows.Count - 1
                grilla_entra.Rows(i).Cells(31).Value = False
            Next
        End If
    End Sub
        ...
        Try
            ...
            For i = 0 To grilla_detalle_venta.Rows.Count - 1
                ...
                If VarCodProducto.Length > 34 Then
                    VarCodProducto = VarCodProducto.Substring(0, 34)
                End If

                If varnombre.Length > 79 Then
                    varnombre = varnombre.Substring(0, 79)
                End If
    ...
    Private Sub DocImprimir_PrintPage(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles Pd.PrintPage
        ...
        e.Graphics.DrawString("CANTIDAD", Font_texto_cabecera, Brushes.Black, margen_izquierdo + 270, margen_superior + 365, format1)
        e.Graphics.DrawLine(Pens.Black, margen_izquierdo + 0, margen_superior + 380, margen_izquierdo + 270, margen_superior + 380)

        'If txt_nombre_producto.Text.Length > 25 Then
        '    txt_nombre_producto.Text = txt_nombre_producto.Text.Substring(0, 25)
        'End If

        Dim Nombre_producto As String = txt_nombre_producto.Text

        If Nombre_producto.Length > 25 Then
            Nombre_producto = Nombre_producto.Substring(0, 25)
        End If

        If txt_numero_tecnico.Text.Length > 25 Then
            txt_numero_tecnico.Text = txt_numero_tecnico.Text.Substring(0, 25)
        End If

        e.Graphics.DrawString(txt_codigo.Text, Font_texto_cabecera, Brushes.Black, margen_izquierdo + 0, margen_superior + 385)
        e.Graphics.DrawString(txt_nombre_producto.Text, Font_texto_cabecera, Brushes.Black, margen_izquierdo + 75, margen_superior + 385)
        ...
        e.Graphics.DrawString(txt_numero_tecnico.Text, Font_texto_cabecera, Brushes.Black, margen_izquierdo + 75, margen_superior + 400)
        e.Graphics.DrawString(txt_cantidad.Text, Font_texto_cabecera, Brushes.Black, margen_izquierdo + 270, margen_superior + 400, format1)

        'e.Graphics.DrawLine(Pens.Black, margen_izquierdo + 10, margen_superior + 500, margen_izquierdo + 100, margen_superior + 500)
        'Dim rect_firma As New Rectangle(margen_izquierdo + 10, margen_superior + 510, margen_izquierdo + 100, margen_superior + 15)
        'e.Graphics.DrawString("FIRMA DESPACHO", Font_texto_titulo_detalle, Brushes.Black, rect_firma, stringFormat)

        'e.Graphics.DrawLine(Pens.Black, margen_izquierdo + 110, margen_superior + 500, margen_izquierdo + 190, margen_superior + 500)
        'Dim rect_firma_2 As New Rectangle(margen_izquierdo + 160, margen_superior + 510, margen_izquierdo + 190, margen_superior + 15)
        'e.Graphics.DrawString("FIRMA RECEPCION", Font_texto_titulo_detalle, Brushes.Black, rect_firma_2, stringFormat)

        e.Graphics.DrawString("_________________", Font_texto_cabecera, Brushes.Black, margen_izquierdo + 10, margen_superior + 485)
        e.Graphics.DrawString("FIRMA DESPACHO", Font_texto_cabecera, Brushes.Black, margen_izquierdo + 10, margen_superior + 500)
        e.Graphics.DrawString("_________________", Font_texto_cabecera, Brushes.Black, margen_izquierdo + 170, margen_superior + 485)
        e.Graphics.DrawString("FIRMA RECEPCION", Font_texto_cabecera, Brushes.Black, margen_izquierdo + 170, margen_superior + 500)



        Dim rect_guion_final As New Rectangle(margen_izquierdo + 10, margen_superior + 550, margen_izquierdo + 250, margen_superior + 15)
