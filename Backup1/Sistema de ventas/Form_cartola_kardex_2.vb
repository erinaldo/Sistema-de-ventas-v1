...
Public Class Form_cartola_kardex_2
    ...
    Sub mostrar_malla()
        ...
        If Check_movimientos.Checked = False Then
            SC.Connection = conexion
            SC.CommandText = "select cod_producto as 'CODIGO', detalle_nombre as 'NOMBRE', detalle_nombre as MOVIMIENTO, nombre_usuario as 'USUARIO', TIPO as TIPO, condiciones as CONDICIONES, detalle_boleta.n_boleta as NUMERO, FECHA_venta as FECHA, cantidad as CANTIDAD, VALOR_UNITARIO as PRECIO from detalle_boleta, boleta, usuarios where boleta.n_boleta=detalle_boleta.n_boleta and estado <> 'ANULADA' and estado <> 'ANULADO' and detalle_boleta.cod_producto= '" & (txt_codigo_producto.Text) & "' AND fecha_venta >='" & (mifecha2) & "' and fecha_venta <= '" & (mifecha4) & "' AND boleta.usuario_responsable=usuarios.rut_usuario order by fecha_venta desc"
            DA.SelectCommand = SC
            DA.Fill(DT)
        Else
            SC.Connection = conexion
        ...
        If Check_movimientos.Checked = False Then
            SC.Connection = conexion
            SC.CommandText = "select cod_producto as 'CODIGO', detalle_nombre as 'NOMBRE', detalle_nombre as MOVIMIENTO, nombre_usuario as 'USUARIO', TIPO as TIPO, condiciones as CONDICIONES, detalle_factura.n_factura as NUMERO, FECHA_venta as FECHA, cantidad as CANTIDAD, VALOR_UNITARIO as PRECIO from detalle_factura, factura, usuarios where factura.usuario_responsable <> 'SISTEMA' and factura.n_factura=detalle_factura.n_factura and estado <> 'ANULADA' and estado <> 'ANULADO' and detalle_factura.cod_producto= '" & (txt_codigo_producto.Text) & "' AND fecha_venta >='" & (mifecha2) & "' and fecha_venta <= '" & (mifecha4) & "' AND factura.usuario_responsable=usuarios.rut_usuario order by fecha_venta desc"
            DA.SelectCommand = SC
            DA.Fill(DT)
        Else
            SC.Connection = conexion
        ...
        If Check_movimientos.Checked = False Then
            SC.Connection = conexion
            SC.CommandText = "select codigo_producto as 'CODIGO', nombre_producto as 'NOMBRE', nombre_usuario as 'USUARIO', vale.n_vale as NUMERO, fecha as FECHA, cantidad as CANTIDAD, valor_unitario as PRECIO from vale, usuarios where estado <> 'ANULADA' and estado <> 'ANULADO' and vale.codigo_producto= '" & (txt_codigo_producto.Text) & "' AND fecha >='" & (mifecha2) & "' and fecha <= '" & (mifecha4) & "' AND vale.usuario_responsable=usuarios.rut_usuario order by fecha desc"
            DA.SelectCommand = SC
            DA.Fill(DT)
        Else
            SC.Connection = conexion
        ...
        If Check_movimientos.Checked = False Then
            SC.Connection = conexion
            SC.CommandText = "select cod_producto as 'CODIGO', detalle_nombre as 'NOMBRE', movimiento as MOVIMIENTO, nombre_usuario as 'USUARIO', TIPO as TIPO, detalle_vale_cambio.nro_vale as NUMERO, fecha as FECHA, cantidad as CANTIDAD, VALOR_UNITARIO as PRECIO from detalle_vale_cambio, vale_cambio, usuarios where vale_cambio.nro_vale=detalle_vale_cambio.nro_vale and estado <> 'ANULADA' and estado <> 'ANULADO' and detalle_vale_cambio.cod_producto= '" & (txt_codigo_producto.Text) & "' AND fecha >='" & (mifecha2) & "' and fecha <= '" & (mifecha4) & "' AND vale_cambio.usuario_responsable=usuarios.rut_usuario order by fecha desc"
            DA.SelectCommand = SC
            DA.Fill(DT)
        Else
            SC.Connection = conexion
        ...
        If Check_movimientos.Checked = False Then
            ...
            SC.CommandText = "select cod_producto as 'CODIGO', nombre as 'NOMBRE', movimiento as MOVIMIENTO, nombre_usuario as 'USUARIO', TIPO as TIPO, envios_recibidos.n_documento as NUMERO, fecha as FECHA, cantidad as CANTIDAD, VALOR_UNITARIO as PRECIO from envios_recibidos, usuarios where envios_recibidos.vendedor=usuarios.rut_usuario and estado <> 'ANULADA' and estado <> 'ANULADO' and cod_producto= '" & (txt_codigo_producto.Text) & "' AND fecha >='" & (mifecha2) & "' and fecha <= '" & (mifecha4) & "' GROUP BY cod_detalle order by fecha desc"
            DA.SelectCommand = SC
            DA.Fill(DT)
        Else
            SC.Connection = conexion
        ...
        If Check_movimientos.Checked = False Then
            SC.Connection = conexion
            'SC.CommandText = "select cod_producto as 'CODIGO', detalle_nombre as 'NOMBRE', detalle_nombre as MOVIMIENTO, nombre_usuario as 'USUARIO', TIPO as TIPO, detalle_factura.n_factura as NUMERO, FECHA_venta as FECHA, cantidad as CANTIDAD, VALOR_UNITARIO as PRECIO from detalle_factura, factura, usuarios where factura.n_factura=detalle_factura.n_factura and estado <> 'ANULADA' and estado <> 'ANULADO' and detalle_factura.cod_producto= '" & (txt_codigo_producto.Text) & "' AND fecha_venta >='" & (mifecha2) & "' and fecha_venta <= '" & (mifecha4) & "' AND factura.usuario_responsable=usuarios.rut_usuario order by fecha_venta desc"
            SC.CommandText = "select cod_producto as 'CODIGO', nombre as 'NOMBRE', movimiento as MOVIMIENTO, nombre_usuario as 'USUARIO', TIPO as TIPO, detalle_consumo_interno.n_documento as NUMERO, fecha as FECHA, cantidad as CANTIDAD, VALOR_UNITARIO as PRECIO from detalle_consumo_interno, usuarios where detalle_consumo_interno.vendedor=usuarios.rut_usuario and estado <> 'ANULADA' and estado <> 'ANULADO' and cod_producto= '" & (txt_codigo_producto.Text) & "' AND fecha >='" & (mifecha2) & "' and fecha <= '" & (mifecha4) & "' GROUP BY cod_detalle order by fecha desc"
            DA.SelectCommand = SC
            DA.Fill(DT)
        Else
            SC.Connection = conexion
        ...
        If Check_movimientos.Checked = False Then
            SC.Connection = conexion
            'SC.CommandText = "select cod_producto as 'CODIGO', detalle_nombre as 'NOMBRE', detalle_nombre as MOVIMIENTO, nombre_usuario as 'USUARIO', TIPO as TIPO, detalle_factura.n_factura as NUMERO, FECHA_venta as FECHA, cantidad as CANTIDAD, VALOR_UNITARIO as PRECIO from detalle_factura, factura, usuarios where factura.n_factura=detalle_factura.n_factura and estado <> 'ANULADA' and estado <> 'ANULADO' and detalle_factura.cod_producto= '" & (txt_codigo_producto.Text) & "' AND fecha_venta >='" & (mifecha2) & "' and fecha_venta <= '" & (mifecha4) & "' AND factura.usuario_responsable=usuarios.rut_usuario order by fecha_venta desc"
            SC.CommandText = "select cod_producto as 'CODIGO', nombre as 'NOMBRE', movimiento as MOVIMIENTO, nombre_usuario as 'USUARIO', TIPO as TIPO, ajustes_de_stock.n_ajuste as NUMERO, fecha as FECHA, cantidad as CANTIDAD, VALOR_UNITARIO as PRECIO from ajustes_de_stock, usuarios where ajustes_de_stock.vendedor=usuarios.rut_usuario and estado <> 'ANULADA' and estado <> 'ANULADO' and cod_producto= '" & (txt_codigo_producto.Text) & "' AND fecha >='" & (mifecha2) & "' and fecha <= '" & (mifecha4) & "' GROUP BY cod_detalle order by fecha desc"
            DA.SelectCommand = SC
            DA.Fill(DT)
        Else
            SC.Connection = conexion
        ...
        If Check_movimientos.Checked = False Then
            SC.Connection = conexion
            SC.CommandText = "select cod_producto as 'CODIGO', nombre as 'NOMBRE', nombre as MOVIMIENTO, nombre_usuario as 'USUARIO', compra.TIPO as TIPO, detalle_compra.n_compra as NUMERO, fecha_emision as FECHA, cantidad as CANTIDAD, valor_unitario as PRECIO from detalle_compra, compra, usuarios where compra.n_compra=detalle_compra.n_compra and compra.rut_proveedor=detalle_compra.rut_proveedor and estado <> 'ANULADA' and estado <> 'ANULADO' and detalle_compra.cod_producto= '" & (txt_codigo_producto.Text) & "' and fecha_emision >='" & (mifecha2) & "' and fecha_emision <= '" & (mifecha4) & "' AND compra.usuario_responsable= usuarios.rut_usuario order by fecha_emision desc "
            DA.SelectCommand = SC
            DA.Fill(DT)
        Else
            SC.Connection = conexion
    ...
    Private Sub PrintDocument1_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ...
        e.Graphics.DrawString("CODIGO", Font_titulo_columna, Brushes.Black, margen_izquierdo + 50, margen_superior + 120)
        e.Graphics.DrawString("NOMBRE", Font_titulo_columna, Brushes.Black, margen_izquierdo + 110, margen_superior + 120)
        e.Graphics.DrawString("MOV.", Font_titulo_columna, Brushes.Black, margen_izquierdo + 310, margen_superior + 120)
        e.Graphics.DrawString("USUARIO", Font_titulo_columna, Brushes.Black, margen_izquierdo + 370, margen_superior + 120)
        e.Graphics.DrawString("TIPO", Font_titulo_columna, Brushes.Black, margen_izquierdo + 510, margen_superior + 120)
        e.Graphics.DrawString("NUMERO", Font_titulo_columna, Brushes.Black, margen_izquierdo + 600, margen_superior + 120)
        e.Graphics.DrawString("FECHA", Font_titulo_columna, Brushes.Black, margen_izquierdo + 670, margen_superior + 120)
        e.Graphics.DrawString("CANT.", Font_titulo_columna, Brushes.Black, margen_izquierdo + 740, margen_superior + 120)
        e.Graphics.DrawString("VALOR", Font_titulo_columna, Brushes.Black, margen_izquierdo + 810, margen_superior + 120)
        ...
        For i = numero_lineas_total To grilla_kardex.Rows.Count - 1
            ...
            e.Graphics.DrawString(Var_codigo, Font_texto_impresion, Brushes.Black, margen_izquierdo + 50, margen_superior + 140 + (numero_lineas * multiplicador_lineas))
            '***************************************************************************************************************************************************

            e.Graphics.DrawString(Var_nombre, Font_texto_impresion, Brushes.Black, margen_izquierdo + 110, margen_superior + 140 + (numero_lineas * multiplicador_lineas))
            '***************************************************************************************************************************************************

            e.Graphics.DrawString(Var_movimiento, Font_texto_impresion, Brushes.Black, margen_izquierdo + 310, margen_superior + 140 + (numero_lineas * multiplicador_lineas))
            '***************************************************************************************************************************************************

            e.Graphics.DrawString(Var_usuario, Font_texto_impresion, Brushes.Black, margen_izquierdo + 370, margen_superior + 140 + (numero_lineas * multiplicador_lineas))
            '***************************************************************************************************************************************************

            e.Graphics.DrawString(Var_tipo, Font_texto_impresion, Brushes.Black, margen_izquierdo + 510, margen_superior + 140 + (numero_lineas * multiplicador_lineas))
            '***************************************************************************************************************************************************

            e.Graphics.DrawString(Var_numero, Font_texto_impresion, Brushes.Black, margen_izquierdo + 650, margen_superior + 140 + (numero_lineas * multiplicador_lineas), format1)
            '***************************************************************************************************************************************************

            e.Graphics.DrawString(Var_fecha, Font_texto_impresion, Brushes.Black, margen_izquierdo + 670, margen_superior + 140 + (numero_lineas * multiplicador_lineas))
            '***************************************************************************************************************************************************

            e.Graphics.DrawString(Var_cantidad, Font_texto_impresion, Brushes.Black, margen_izquierdo + 780, margen_superior + 140 + (numero_lineas * multiplicador_lineas), format1)
            '***************************************************************************************************************************************************

            e.Graphics.DrawString(Var_valor, Font_texto_impresion, Brushes.Black, margen_izquierdo + 850, margen_superior + 140 + (numero_lineas * multiplicador_lineas), format1)
            '***************************************************************************************************************************************************
