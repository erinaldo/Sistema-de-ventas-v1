...
Public Class Form_venta
    ...
    Private Sub Form_venta_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If mirecintoempresa = "BDO. O´HIGGINS 441" Then
            ...
            Picture_call_center.Location = New Point(285, 5)
            lbl_asistencia.Location = New Point(348, 5)
    ...
    Sub grabar_letras()
        ...
        If Combo_venta.Text = "BOLETA" Then
            SC.Connection = conexion
            SC.CommandText = "insert into creditos (n_creditos, tipo, tipo_detalle, rut_cliente, codigo_cliente, fecha_venta, descuento, neto, iva, subtotal, total, saldo, desglose, condiciones, estado, usuario_responsable, codigo_afecta, nombre_afecta, recinto) values (" & (txt_nro_letra.Text) & ",'" & ("LETRA") & "','" & ("LETRA") & "','" & (txt_rut_cliente.Text) & "','" & (txt_cod_cliente.Text) & "','" & (dtp_letra.Text) & "'," & (txt_desc.Text) & "," & (Neto_letra) & "," & (Iva_letra) & "," & (Total_letra) & "," & (Total_letra) & "," & (Total_letra) & ",'-','" & (combo_condiciones.Text) & "','" & ("EMITIDA") & "','" & (txt_rut_vendedor.Text) & "','" & (txt_factura.Text) & "','BOLETA','" & (mirecintoempresa) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)

            SC.Connection = conexion
            SC.CommandText = "insert into letras (n_letra, cant_letras, doc_referencia, nro_referencia, fecha, rut_cliente, monto_letra, total_letra, estado, usuario_responsable) values (" & (txt_nro_letra.Text) & " , '" & (txt_cargar.Text) & "', '" & (Combo_venta.Text) & "','" & (txt_factura.Text) & "','" & (dtp_fecha.Text) & "','" & (txt_rut_cliente.Text) & "'," & (Total_letra) & "," & (txt_total.Text) & ", 'EMITIDA', '" & (txt_rut_vendedor.Text) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)
        End If
        ...
        If Combo_venta.Text = "FACTURA" Then
            SC.Connection = conexion
            SC.CommandText = "insert into creditos (n_creditos, tipo, tipo_detalle, rut_cliente, codigo_cliente, fecha_venta, descuento, neto, iva, subtotal, total, saldo, desglose, condiciones, estado, usuario_responsable, codigo_afecta, nombre_afecta, recinto) values (" & (txt_factura.Text) & ",'" & ("LETRA") & "','" & ("LETRA") & "','" & (txt_rut_cliente.Text) & "','" & (txt_cod_cliente.Text) & "','" & (dtp_letra.Text) & "'," & (txt_desc.Text) & "," & (Neto_letra) & "," & (Iva_letra) & "," & (Total_letra) & "," & (Total_letra) & "," & (Total_letra) & ",'-','" & (combo_condiciones.Text) & "','" & ("EMITIDA") & "','" & (txt_rut_vendedor.Text) & "','" & (txt_factura.Text) & "','FACTURA','" & (mirecintoempresa) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)
    ...
    Sub cargar_documento()
        ...
        If Combo_venta.Text = "ASISTENCIA" Then
            ...
            DS.Clear()
            SC.Connection = conexion
            SC.CommandText = "select * from detalle_asistencias where detalle_asistencias.n_asistencia='" & (txt_cargar.Text) & "'"
            DA.SelectCommand = SC
            DA.Fill(DT)
            DS.Tables.Add(DT)
            ...
            If DS.Tables(DT.TableName).Rows.Count > 0 Then
                ...
                SC.Connection = conexion
                SC.CommandText = "select * from clientes, asistencias where cod_cliente = asistencias.codigo_cliente and asistencias.n_asistencia='" & (txt_cargar.Text) & "'"
                DA.SelectCommand = SC
                DA.Fill(DT)
                DS.Tables.Add(DT)
                ...
                If DS.Tables(DT.TableName).Rows.Count > 0 Then
                    ...
                    txt_tipo_documento.Text = DS.Tables(DT.TableName).Rows(0).Item("tipo_cuenta")
                    txt_descuento_cliente_2.Text = DS.Tables(DT.TableName).Rows(0).Item("descuento_2")
                    txt_nro_cotizacion.Text = DS.Tables(DT.TableName).Rows(0).Item("n_asistencia")
                    txt_rut_asistente.Text = DS.Tables(DT.TableName).Rows(0).Item("usuario_responsable")
                    txt_pagare.Text = DS.Tables(DT.TableName).Rows(0).Item("pagare")
                    Combo_nivel_asistencia.Text = DS.Tables(DT.TableName).Rows(0).Item("nivel_asistencia")
                    txt_saldo.Text = Val(DS.Tables(DT.TableName).Rows(0).Item("cupo_cliente")) - Val(DS.Tables(DT.TableName).Rows(0).Item("saldo_cliente"))
                    ...
                    txt_rut_asistente.Visible = True
                    Combo_nivel_asistencia.Visible = True
                    ...
                    SC.Connection = conexion
                    SC.CommandText = "select nombre_usuario from usuarios where rut_usuario ='" & (txt_rut_asistente.Text) & "'"
                    DA.SelectCommand = SC
                    DA.Fill(DT)
                    DS.Tables.Add(DT)
    ...
    Sub condiciones()
        If mirutempresa = "81921000-4" Then
            ...
            If Combo_venta.Text = "COTIZACION" Then
                ...
                combo_condiciones.Items.Add("CREDITO")
                combo_condiciones.Items.Add("OTRO MEDIO DE PAGO")
                combo_condiciones.SelectedItem = "EFECTIVO"
            End If
        End If
    ...
    Sub limpiar()
        ...
        If mirutempresa <> "87686300-6" Then
            ...
            txt_rut_asistente.Visible = False
            Combo_nivel_asistencia.Visible = False
        Else
            txt_nro_cotizacion.Visible = True
            txt_rut_asistente.Visible = True
            Combo_nivel_asistencia.Visible = True
    ...
    Sub crear_numero_documento()
        ...
        If Combo_venta.Text = "ASISTENCIA" Then
            ...
            Try
                SC.Connection = conexion
                SC.CommandText = "select max(cod_auto) as cod_auto from asistenciaS"
                DA.SelectCommand = SC
                DA.Fill(DT)
                DS.Tables.Add(DT)
                If DS.Tables(DT.TableName).Rows.Count > 0 Then
            ...
            Try
                SC.Connection = conexion
                SC.CommandText = "select n_asistencia from asistencia where cod_auto='" & (varnumdoc) & "'"
                DA.SelectCommand = SC
                DA.Fill(DT)
                DS.Tables.Add(DT)
                If DS.Tables(DT.TableName).Rows.Count > 0 Then
                    varnumdoc = DS.Tables(DT.TableName).Rows(0).Item("n_asistencia")
                    txt_factura.Text = varnumdoc + 1
                End If
            Catch err As InvalidCastException
                txt_factura.Text = 1
            ...
            If estado_boleta_electronica = "NO" Then
                With Pd.PrinterSettings
                    ...
                    If .IsValid Then
                        ...
                        lbl_mensaje.Visible = False
                        Me.Enabled = True
                        'If Form_calcular.Visible = False Then
                        MsgBox("SE HA IMPRESO CORRECTAMENTE", MessageBoxIcon.Information + MsgBoxStyle.OkOnly, "ATENCION")
                        'End If
                        Exit Sub
                    Else
    ...
    Sub grabar_detalle_factura()
        ...
        If Combo_venta.Text = "FACTURA DE CREDITO" Then
            For i = 0 To grilla_detalle_venta.Rows.Count - 1
                ...
                SC.Connection = conexion
                SC.CommandText = "insert into detalle_total (n_total, tipo, cod_producto, nombre, valor_unitario, cantidad, descuento, neto, iva, subtotal, total, movimiento, fecha, vendedor, estado) values(" & (txt_factura.Text) & ",'FACTURA DE CREDITO', '" & (VarCodProducto) & "','" & (varnombre) & "'," & (VarValorUnitario) & ",'" & (VarCantidad) & "'," & (VarDescuento) & "," & (VarNeto) & ", " & (VarIva) & "," & (VarSubtotal) & "," & (VarTotal) & ",'SALE','" & (dtp_fecha.Text) & "', '" & (txt_rut_vendedor.Text) & "' ,'EMITIDA')"
                DA.SelectCommand = SC
                DA.Fill(DT)
            Next
        End If
        ...
        If Combo_venta.Text = "GUIA" Then
            For i = 0 To grilla_detalle_venta.Rows.Count - 1
                ...
                SC.Connection = conexion
                SC.CommandText = "insert into detalle_total (n_total, tipo, cod_producto, nombre, valor_unitario, cantidad, descuento, neto, iva, subtotal, total, movimiento, fecha, vendedor, estado) values(" & (txt_factura.Text) & ",'GUIA', '" & (VarCodProducto) & "','" & (varnombre) & "'," & (VarValorUnitario) & ",'" & (VarCantidad) & "'," & (VarDescuento) & "," & (VarNeto) & ", " & (VarIva) & "," & (VarSubtotal) & "," & (VarTotal) & ",'SALE','" & (dtp_fecha.Text) & "', '" & (txt_rut_vendedor.Text) & "' ,'EMITIDA')"
                DA.SelectCommand = SC
                DA.Fill(DT)
            Next
        End If
        ...
        If Combo_venta.Text = "ASISTENCIA" Then
            For i = 0 To grilla_detalle_venta.Rows.Count - 1
                ...
                SC.Connection = conexion
                SC.CommandText = "insert into detalle_asistencias (n_asistencia, cod_producto, detalle_nombre, numero_tecnico, valor_unitario, cantidad, porcentaje_desc, detalle_descuento, detalle_neto, detalle_iva, detalle_subtotal, detalle_total) values(" & (txt_factura.Text) & ",'" & (VarCodProducto) & "','" & (varnombre) & "','" & (vartecnico) & "'," & (VarValorUnitario) & ",'" & (VarCantidad) & "'," & (VarPorcentaje) & ", " & (VarDescuento) & "," & (VarNeto) & ", " & (VarIva) & "," & (VarSubtotal) & "," & (VarTotal) & ")"
                DA.SelectCommand = SC
                DA.Fill(DT)
            Next
        End If
        ...
        If Combo_venta.Text = "BOLETA" Then
            For i = 0 To grilla_detalle_venta.Rows.Count - 1
                ...
                If VarTipoMedida = "DECIMAL" Then
                    ...
                    cantidad_decimal = (cantidad_decimal) - (VarCantidad)
                ...
                SC.Connection = conexion
                SC.CommandText = "insert into detalle_total (n_total, tipo, cod_producto, nombre, valor_unitario, cantidad, descuento, neto, iva, subtotal, total, movimiento, fecha, vendedor, estado) values(" & (txt_factura.Text) & ",'BOLETA', '" & (VarCodProducto) & "','" & (varnombre) & "'," & (VarValorUnitario) & ",'" & (VarCantidad) & "'," & (VarDescuento) & "," & (VarNeto) & ", " & (VarIva) & "," & (VarSubtotal) & "," & (VarTotal) & ",'SALE','" & (dtp_fecha.Text) & "', '" & (txt_rut_vendedor.Text) & "' ,'EMITIDA')"
                DA.SelectCommand = SC
                DA.Fill(DT)
            Next
        End If
        ...
        If Combo_venta.Text = "BOLETA DE CREDITO" Then
            For i = 0 To grilla_detalle_venta.Rows.Count - 1
                ...
                SC.Connection = conexion
                SC.CommandText = "insert into detalle_total (n_total, tipo, cod_producto, nombre, valor_unitario, cantidad, descuento, neto, iva, subtotal, total, movimiento, fecha, vendedor, estado) values(" & (txt_factura.Text) & ",'BOLETA DE CREDITO', '" & (VarCodProducto) & "','" & (varnombre) & "'," & (VarValorUnitario) & ",'" & (VarCantidad) & "'," & (VarDescuento) & "," & (VarNeto) & ", " & (VarIva) & "," & (VarSubtotal) & "," & (VarTotal) & ",'SALE','" & (dtp_fecha.Text) & "', '" & (txt_rut_vendedor.Text) & "' ,'EMITIDA')"
                DA.SelectCommand = SC
                DA.Fill(DT)
            Next
        End If
    ...
    Sub grabar_cotizacion()
        SC.Connection = conexion
        SC.CommandText = "insert into cotizacion (n_cotizacion, tipo, rut_cliente, codigo_cliente, fecha_venta, descuento, neto, iva, subtotal, total, condiciones, usuario_responsable, hora, porcentaje_desc, nivel_asistencia) values (" & (txt_nro_cotizacion.Text) & " , '" & ("COTIZACION") & "', '11111111-1','0','" & (dtp_fecha.Text) & "'," & (txt_desc.Text) & "," & (txt_neto.Text) & "," & (txt_iva.Text) & "," & (txt_sub_total.Text) & "," & (txt_total.Text) & ",'" & (combo_condiciones.Text) & "','" & (txt_rut_asistente.Text) & "','" & (Form_menu_principal.lbl_hora.Text) & "','" & (txt_porcentaje_desc.Text) & "','" & (Combo_nivel_asistencia.Text) & "')"
        DA.SelectCommand = SC
        DA.Fill(DT)
    End Sub
    ...
    Sub grabar_factura()
        ...
        If txt_rut_asistente.Text <> "-" And Combo_nivel_asistencia.Text <> "-" And Combo_venta.Text <> "COTIZACION" Then
            If txt_nro_cotizacion.Text = "-" Or txt_nro_cotizacion.Text = "" Then
                crear_numero_cotizacion()
                grabar_cotizacion()
                grabar_detalle_cotizacion()

                SC.Connection = conexion
                SC.CommandText = "UPDATE cotizacion SET nro_referencia='" & (txt_factura.Text) & "', doc_referencia='" & (Combo_venta.Text) & "', nivel_asistencia_vendedor='" & (Combo_nivel_asistencia.Text) & "', nombre_vendedor='" & (minombre) & "' WHERE n_cotizacion='" & (txt_nro_cotizacion.Text) & "'"
                DA.SelectCommand = SC
                DA.Fill(DT)
        ...
        If Combo_venta.Text = "COTIZACION" Then
            SC.Connection = conexion
            SC.CommandText = "insert into cotizacion (n_cotizacion, tipo, rut_cliente, codigo_cliente, fecha_venta, descuento, neto, iva, subtotal, total, condiciones, usuario_responsable, hora, porcentaje_desc, nivel_asistencia) values (" & (txt_factura.Text) & " , '" & ("COTIZACION") & "', '" & (txt_rut_cliente.Text) & "','" & (txt_cod_cliente.Text) & "','" & (dtp_fecha.Text) & "'," & (txt_desc.Text) & "," & (txt_neto.Text) & "," & (txt_iva.Text) & "," & (txt_sub_total.Text) & "," & (txt_total.Text) & ",'" & (condicion_doc) & "','" & (txt_rut_vendedor.Text) & "','" & (Form_menu_principal.lbl_hora.Text) & "','" & (txt_porcentaje_desc.Text) & "','" & (Combo_nivel_asistencia.Text) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)
        End If
        ...
        If Combo_venta.Text = "GUIA" Then
            ...
            SC.Connection = conexion
            SC.CommandText = "insert into guia_comisiones (n_guia, fecha_venta, porcentaje_desc, descuento, total, estado, usuario_responsable) values (" & (txt_factura.Text) & ", '" & (dtp_fecha.Text) & "', '" & (porcentaje_desc_guia) & "', " & (descuento_guia) & ", " & (total_guia) & ", 'SIN FACTURAR', '" & (txt_rut_vendedor.Text) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)

            SC.Connection = conexion
            SC.CommandText = "insert into guia (caja, n_guia, tipo, rut_cliente, codigo_cliente, fecha_venta, descuento, neto, iva, subtotal, total, condiciones, estado, usuario_responsable,rut_retira, nombre_retira, recinto, orden, hora, porcentaje_desc, tipo_impresion, vehiculo, patente, pie, condicion_de_pago_pie, comision) values ('" & (nombre_pc) & "', " & (txt_factura.Text) & " , '" & ("GUIA") & "', '" & (txt_rut_cliente.Text) & "','" & (txt_cod_cliente.Text) & "','" & (dtp_fecha.Text) & "'," & (txt_desc.Text) & "," & (txt_neto.Text) & "," & (txt_iva.Text) & "," & (txt_sub_total.Text) & "," & (txt_total.Text) & ",'" & (condicion_doc) & "','" & ("SIN FACTURAR") & "','" & (txt_rut_vendedor.Text) & "','" & (txt_rut_retira.Text) & "','" & (txt_nombre_retira.Text) & "','" & (mirecintoempresa) & "','" & (txt_nro_orden_de_compra.Text) & "','" & (Form_menu_principal.lbl_hora.Text) & "','" & (txt_porcentaje_desc.Text) & "','" & (tipo_impresion) & "', '-', '-', '" & (valor_pie) & "', '" & (tipo_pie) & "', '" & (total_comision) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)
        End If
        ...
        If Combo_venta.Text = "FACTURA DE CREDITO" Then
            ...
            SC.Connection = conexion
            SC.CommandText = "insert into factura (caja, n_factura, tipo, rut_cliente, codigo_cliente, fecha_venta, descuento, neto, iva, subtotal, total, condiciones, estado, usuario_responsable, rut_retira, nombre_retira, recinto, orden, hora, porcentaje_desc, tipo_impresion, pie, condicion_de_pago_pie, comision) values ('" & (nombre_pc) & "', " & (txt_factura.Text) & ",'" & ("FACTURA") & "','" & (txt_rut_cliente.Text) & "','" & (txt_cod_cliente.Text) & "', '" & (dtp_fecha.Text) & "'," & (txt_desc.Text) & "," & (txt_neto.Text) & "," & (txt_iva.Text) & "," & (txt_sub_total.Text) & "," & (txt_total.Text) & ",'" & (condicion_doc) & "','" & ("EMITIDA") & "','" & (txt_rut_vendedor.Text) & "','" & (txt_rut_retira.Text) & "','" & (txt_nombre_retira.Text) & "','" & (mirecintoempresa) & "','" & (txt_nro_orden_de_compra.Text) & "','" & (Form_menu_principal.lbl_hora.Text) & "','" & (txt_porcentaje_desc.Text) & "','" & (tipo_impresion) & "', '" & (valor_pie) & "', '" & (tipo_pie) & "', '" & (total_comision) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)

            SC.Connection = conexion
            SC.CommandText = "insert into creditos (n_creditos, tipo, tipo_detalle, rut_cliente, codigo_cliente, fecha_venta, descuento, neto, iva, subtotal, total, saldo, condiciones, estado, usuario_responsable, codigo_afecta, nombre_afecta, recinto, fecha_vencimiento, fecha_pago, numero_cuota, total_cuotas, interes, gastos_de_cobranza, pie, convenio, condicion_de_pago_pie) values (" & (txt_factura.Text) & ",'" & ("FACTURA") & "','" & ("FACTURA") & "','" & (txt_rut_cliente.Text) & "','" & (txt_cod_cliente.Text) & "','" & (dtp_fecha.Text) & "'," & (txt_desc.Text) & "," & (txt_neto.Text) & "," & (txt_iva.Text) & "," & (txt_sub_total.Text) & "," & (txt_total.Text) & "," & (txt_total.Text) & ",'" & (combo_condiciones.Text) & "','" & ("EMITIDA") & "','" & (txt_rut_vendedor.Text) & "','" & (txt_factura.Text) & "','FACTURA', '" & (mirecintoempresa) & "', '" & (dtp_vencimiento.Text) & "', '0000-00-00', '1', '1', '0', '0' , '" & (valor_pie) & "', '" & (txt_codigo_convenio.Text) & "' , '" & (tipo_pie) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)
        ...
        If Combo_venta.Text = "FACTURA" Then
            ...
            SC.Connection = conexion
            SC.CommandText = "insert into factura(caja, n_factura, tipo, rut_cliente, codigo_cliente, fecha_venta, descuento, neto, iva, subtotal, total,condiciones,estado,usuario_responsable,rut_retira, nombre_retira, recinto, hora, porcentaje_desc, orden, tipo_impresion, pie, condicion_de_pago_pie, comision)values('" & (nombre_pc) & "', " & (txt_factura.Text) & ",'" & ("FACTURA") & "','" & (txt_rut_cliente.Text) & "','" & (txt_cod_cliente.Text) & "','" & (dtp_fecha.Text) & "'," & (txt_desc.Text) & "," & (txt_neto.Text) & "," & (txt_iva.Text) & "," & (txt_sub_total.Text) & "," & (txt_total.Text) & ",'" & (condicion_doc) & "','" & ("EMITIDA") & "','" & (txt_rut_vendedor.Text) & "','" & (txt_rut_retira.Text) & "','" & (txt_nombre_retira.Text) & "','" & (mirecintoempresa) & "','" & (Form_menu_principal.lbl_hora.Text) & "','" & (txt_porcentaje_desc.Text) & "', '0','" & (tipo_impresion) & "', '" & (valor_pie) & "', '" & (tipo_pie) & "', '" & (total_comision) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)
        End If
        ...
        If Combo_venta.Text = "BOLETA DE CREDITO" Then
            ...
            SC.Connection = conexion
            SC.CommandText = "insert into BOLETA (caja, n_boleta, tipo, rut_cliente, codigo_cliente, fecha_venta, descuento, neto, iva, subtotal, total, condiciones, estado, usuario_responsable, hora, porcentaje_desc, tipo_impresion, pie, condicion_de_pago_pie, comision) values ('" & (nombre_pc) & "', " & (txt_factura.Text) & " , 'BOLETA DE CREDITO', '" & (txt_rut_cliente.Text) & "','" & (txt_cod_cliente.Text) & "','" & (dtp_fecha.Text) & "'," & (txt_desc.Text) & "," & (txt_neto.Text) & "," & (txt_iva.Text) & "," & (txt_sub_total.Text) & "," & (txt_total.Text) & ",'" & (condicion_doc) & "','" & ("EMITIDA") & "','" & (txt_rut_vendedor.Text) & "','" & (Form_menu_principal.lbl_hora.Text) & "','" & (txt_porcentaje_desc.Text) & "','" & (tipo_impresion) & "', '" & (valor_pie) & "', '" & (tipo_pie) & "', '" & (total_comision) & "')"
            DA.SelectCommand = SC
            DA.Fill(DT)
    ...
    Private Sub btn_agregar_retira_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_agregar_retira.LostFocus
        btn_agregar_retira.BackColor = Color.Transparent
    End Sub
    ...
    Private Sub DocImprimir_PrintPage(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles Pd.PrintPage
        ...
        If Combo_venta.Text = "BOLETA" Then
            ...
            If mirutempresa <> "81921000-4" Then
                e.Graphics.DrawString(dtp_fecha.Text & " " & mirecintoempresa, Font10, Brushes.Black, margen + 85, 5)
    ...
    Sub revisar_vuelto()
        If vuelto = "SI" Then
            ...
            Me.txt_efectivo.Location = New Point(86, 28)
            Me.txt_vuelto.Location = New Point(86, 59)
            Me.txt_neto_millar.Location = New Point(86, 28)
            Me.txt_iva_millar.Location = New Point(86, 59)
            Me.txt_total_millar.Location = New Point(86, 90)
        End If
        ...
        If vuelto = "NO" Then
            ...
            Me.txt_neto_millar.Location = New Point(77, 28)
            Me.txt_iva_millar.Location = New Point(77, 59)
            Me.txt_total_millar.Location = New Point(77, 90)
        End If
    End Sub
    ...
    Sub crear_archivo_plano()
        ...
        If Combo_venta.Text = "FACTURA" Then
            Try
                ...
                & "33" & ";" & (txt_factura.Text) & ";" & (dtp_fecha.Text) & ";" & "0" & ";" & "0" & ";" & (txt_rut_cliente.Text) & ";" & (nombre_cliente) & ";" & (giro_cliente) & ";" & (direccion_cliente) & ";" & (ciudad_cliente) & ";" & (comuna_cliente) & ";" & (correo_cliente) & ";" & (condicion) & ";" & (txt_nombre_usuario.Text) & ";" & (txt_rut_retira.Text) & ";" & (txt_nombre_retira.Text) & ";" & (mirecintoempresa) & ";" & (telefono_cliente) & ";" & vbNewLine _
                & "->Totales<-" & vbNewLine _
        ...
        If Combo_venta.Text = "GUIA" Then
            ...
            Try
                ...
                'escribimos en el archivo
                strStreamWriter.WriteLine("->Encabezado<-" & vbNewLine _
                & "52" & ";" & (txt_factura.Text) & ";" & (dtp_fecha.Text) & ";" & (tipo_despacho) & ";" & (IndTraslado) & ";" & (txt_rut_cliente.Text) & ";" & (nombre_cliente) & ";" & (giro_cliente) & ";" & (direccion_cliente) & ";" & (ciudad_cliente) & ";" & (comuna_cliente) & ";" & (correo_cliente) & ";" & (condicion) & ";" & (txt_nombre_usuario.Text) & ";" & (txt_rut_retira.Text) & ";" & (txt_nombre_retira.Text) & ";" & (mirecintoempresa) & ";" & (telefono_cliente) & ";" & vbNewLine _
                & "->Totales<-" & vbNewLine _
                & (0) & ";" & (txt_desc_total.Text) & ";" & "0" & ";" & "0" & ";" & (txt_neto.Text) & ";" & "0" & ";" & (valor_iva) & ";" & (txt_iva.Text) & ";" & (txt_total.Text) & ";1;" & vbNewLine _
                & "->Detalle<-")
    ...
    Private Sub PictureBox1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Picture_call_center.Click
        Me.Enabled = False
    ...
    Private Sub Picture_call_center_MouseLeave(ByVal sender As Object, ByVal e As System.EventArgs) Handles Picture_call_center.MouseLeave
        lbl_asistencia.ForeColor = Color.Black
    End Sub

    Private Sub Picture_call_center_MouseMove(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles Picture_call_center.MouseMove
        lbl_asistencia.ForeColor = Color.White
    End Sub
