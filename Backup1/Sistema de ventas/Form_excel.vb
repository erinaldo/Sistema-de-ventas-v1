...
Public Class Form_excel
    ...
    Private Sub Form_excel_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If miusuario <> "16972940-9" Then
            ...
            If mirutempresa = "81921000-4" Then
                combo_tipo.Items.Clear()
                combo_tipo.Items.Add("ACTUALIZAR CENTRAL DE COSTOS")
            End If
        End If
    End Sub
    ...
    Private Sub btn_guardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_guardar.Click
        ...
        If combo_tipo.Text = "ACTUALIZAR CENTRAL DE COSTOS" Then
            For i = 0 To grilla_excel.Rows.Count - 1
                rut_proveedor = grilla_excel.Rows(i).Cells(0).Value.ToString
                fecha = grilla_excel.Rows(i).Cells(1).Value.ToString
                codigo_interno = grilla_excel.Rows(i).Cells(2).Value.ToString
                codigo_barra = grilla_excel.Rows(i).Cells(3).Value.ToString
                marca = grilla_excel.Rows(i).Cells(4).Value.ToString
                nombre = grilla_excel.Rows(i).Cells(5).Value.ToString
                numero_tecnico = grilla_excel.Rows(i).Cells(6).Value.ToString
                aplicacion = grilla_excel.Rows(i).Cells(7).Value.ToString
                nombre_familia = grilla_excel.Rows(i).Cells(8).Value.ToString
                costo = grilla_excel.Rows(i).Cells(9).Value.ToString
                precio = grilla_excel.Rows(i).Cells(10).Value.ToString
                precio_venta_sugerido = grilla_excel.Rows(i).Cells(11).Value.ToString
                margen_1 = grilla_excel.Rows(i).Cells(12).Value.ToString
                margen_2 = grilla_excel.Rows(i).Cells(13).Value.ToString
                margen_3 = grilla_excel.Rows(i).Cells(14).Value.ToString
                margen_4 = grilla_excel.Rows(i).Cells(15).Value.ToString
                margen_5 = grilla_excel.Rows(i).Cells(16).Value.ToString
                maestra = grilla_excel.Rows(i).Cells(17).Value.ToString

                Dim valor As Integer
                valor = codigo_interno
                codigo_interno = String.Format("{0:000000}", valor)
                ...
                SC.Connection = conexion
                SC.CommandText = "INSERT INTO central_de_costos(proveedor, fecha, codigo_interno, codigo_barra, marca, nombre, numero_tecnico, aplicacion, familia, costo, precio, precio_venta_sugerido, margen_1, margen_2, margen_3, margen_4, margen_5, maestra) VALUES ('" & (rut_proveedor) & "','" & (fecha) & "','" & (codigo_interno) & "','" & (codigo_barra) & "','" & (marca) & "','" & (nombre) & "','" & (numero_tecnico) & "','" & (aplicacion) & "','" & (nombre_familia) & "','" & (costo) & "','" & (precio) & "','" & (precio_venta_sugerido) & "','" & (margen_1) & "','" & (margen_2) & "','" & (margen_3) & "','" & (margen_4) & "','" & (margen_5) & "','" & (maestra) & "')"
                DA.SelectCommand = SC
                DA.Fill(DT)
            Next
        End If
        ...
        If combo_tipo.Text = "ACTUALIZAR NOMBRE" Then
            For i = 0 To grilla_excel.Rows.Count - 1
                ...
                SC.Connection = conexion
                SC.CommandText = "UPDATE productos SET nombre='" & (nombre) & "' WHERE cod_producto = '" & (cod_producto) & "'"
                DA.SelectCommand = SC
                DA.Fill(DT)
            Next
        End If
